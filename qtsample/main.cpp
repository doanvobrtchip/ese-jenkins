...
void EmulatorThread::run()
{
	...
	params.Setup = setup;
	params.Loop = loop;
	params.Graphics = FTQT::ftqtGraphics;
	params.Flags = 
...
EmulatorThread *g_EmulatorThread = NULL;
void ftqtRepaint()
{
	g_EmulatorThread->repaint();
}
...
int main(int argc, char* argv[])
{
	...
	g_EmulatorThread = new EmulatorThread();
	g_EmulatorThread->start();
	FTQT::MainWindow mainWin(customSizeHints);
	mainWin.resize(800, 600);
	mainWin.show(); // calls isVisible(true)
	int result = app.exec();
	delete g_EmulatorThread;
	g_EmulatorThread = NULL;
#ifdef NL_OS_WINDOWS
