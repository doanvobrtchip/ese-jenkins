...
TI2CMBasicConfig I2CMConfig;
TI2SBasicConfig I2SConfig;
...
ft_void_t Ft_App_WrCoStr_Buffer(Ft_Gpu_Hal_Context_t *phost,
		const ft_char8_t *s) {
#ifdef  BUFFER_OPTIMIZATION  
...
ft_void_t FT900_Config() {
	...
	UARTSysInit(UART0);
	// Configure UART0
	UART0Config.mode = UART_MODE_450;
	UART0Config.dataBits = UART_DATA_BITS_8;
	UART0Config.stopBits = UART_STOP_BITS_1;
	UART0Config.parity = UART_PARITY_NONE;
	UART0Config.flowControl = UART_FLOW_CONTROL_NONE;
	UART0Config.baudrate = 115200;
	UART0Config.clk = 100000000;
	d16950BasicInit(UART0, &UART0Config);
	...
	SPIMConfig.ratio = DQSPI_CR4;           			// CLK/8
	...
	//useful for filesystem
#if 1
	// Initialize SD Host HW
	SDHOSTSysInit();
	...
	SDHostStatus = sdhostCardDetect();
	if (SDHostStatus == SDHOST_CARD_INSERTED) {
		printf("\nCard inserted!\n");
	} else if (SDHostStatus == SDHOST_CARD_REMOVED) {
		printf("\nPlease insert card\n");
		do {
			SDHostStatus = sdhostCardDetect();
		} while (SDHostStatus == SDHOST_CARD_REMOVED);
		printf("\nCard inserted!\n");
	}

	// Create a work area on the current card (device 0)
	fResult = f_mount(0, &FatFs);

	if (fResult != FR_OK) {
		printf("Failure in f_mount %d \n", fResult);
	}
...
ft_void_t SAMAPP_GPU_Points() {
	...
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 20,
			VERTEX2F((FT_DispWidth / 5) * 16, (FT_DispHeight / 2) * 16));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 24, COLOR_RGB(0, 128, 0));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 28, POINT_SIZE(15 * 16));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 32,
			VERTEX2F((FT_DispWidth * 2 / 5) * 16, (FT_DispHeight / 2) * 16));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 36, COLOR_RGB(0, 0, 128));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 40, POINT_SIZE(25 * 16));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 44,
			VERTEX2F((FT_DispWidth * 3 / 5) * 16, (FT_DispHeight / 2) * 16));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 48, COLOR_RGB(128, 128, 0));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 52, POINT_SIZE(35 * 16));
	Ft_Gpu_Hal_Wr32(phost, RAM_DL + 56,
			VERTEX2F((FT_DispWidth * 4 / 5) * 16, (FT_DispHeight / 2) * 16));
...
ft_void_t SAMAPP_GPU_Fonts() {
	...
	for (i = 0; i < SAMAPP_NUM_OF_ROM_FONTS; i++) {
		...
		Ft_Gpu_Hal_RdMem(phost, (FontTableAddress + i * FT_GPU_FONT_TABLE_SIZE),
				(ft_uint8_t *) &Display_fontstruct,
...
ft_void_t SAMAPP_CoPro_Widget_Calibrate() {
	...
	Ft_Gpu_CoCmd_Text(phost,(FT_DispWidth/2), (FT_DispHeight/2), 27, OPT_CENTER, "Please Tap on the dot");
		...
		Ft_Gpu_Hal_RdMem(phost,
		REG_TOUCH_TRANSFORM_A, (ft_uint8_t *) TransMatrix,
...
ft_void_t SAMAPP_CoPro_AppendCmds() {
	...
	AppendCmds[5] = BITMAP_LAYOUT(SAMAPP_Bitmap_RawData_Header[0].Format,SAMAPP_Bitmap_RawData_Header[0].Stride,SAMAPP_Bitmap_RawData_Header[0].Height);
	AppendCmds[6] = BITMAP_SIZE(BILINEAR, BORDER, BORDER,SAMAPP_Bitmap_RawData_Header[0].Width,	SAMAPP_Bitmap_RawData_Header[0].Height);
...
ft_void_t SAMAPP_CoPro_Widget_Clock() {
	...
	/*************************************************************************/
	{
		...
		Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(0));
		Ft_App_WrCoCmd_Buffer(phost, BITMAP_LAYOUT(SAMAPP_Bitmap_RawData_Header[0].Format, 
						SAMAPP_Bitmap_RawData_Header[0].Stride,
						SAMAPP_Bitmap_RawData_Header[0].Height));
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_SIZE(BILINEAR, BORDER, BORDER, 2 * cRadius, 2 * cRadius));
		Ft_App_WrCoCmd_Buffer(phost,
				VERTEX2F((xOffset - cRadius) * 16, (yOffset - cRadius) * 16));
		Ft_App_WrCoCmd_Buffer(phost, END());
...
ft_void_t SAMAPP_CoPro_Widget_Guage() {
	...
	/*************************************************************************/
	{
		...
		/* Lena bitmap - scale proportionately wrt output resolution */
		Ft_Gpu_CoCmd_LoadIdentity(phost);
		Ft_Gpu_CoCmd_Scale(phost,
				65536 * 2 * cRadius / SAMAPP_Bitmap_RawData_Header[0].Width,
		...
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_LAYOUT(SAMAPP_Bitmap_RawData_Header[0].Format,
						...
						SAMAPP_Bitmap_RawData_Header[0].Height));
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_SIZE(BILINEAR, BORDER, BORDER, 2 * cRadius,
						2 * cRadius));
		Ft_App_WrCoCmd_Buffer(phost,
				VERTEX2F((xOffset - cRadius) * 16, (yOffset - cRadius) * 16));
		Ft_App_WrCoCmd_Buffer(phost, END());
...
ft_void_t SAMAPP_CoPro_Widget_Gradient() {
	...
	/*************************************************************************/
	{
		...
		Ft_Gpu_CoCmd_Gradient(phost, xOffset, (yOffset + hScissor / 2 - 15),
				0x606060, (xOffset + wScissor), (yOffset + hScissor / 2 + 15),
				0x404080);
		...
		Ft_App_WrCoCmd_Buffer(phost, SCISSOR_SIZE(wScissor, hScissor));
		Ft_App_WrCoCmd_Buffer(phost, SCISSOR_XY(xOffset, yOffset)); //clip the display
		Ft_Gpu_CoCmd_Gradient(phost, xOffset, yOffset, 0x808080,
				(xOffset + wScissor), yOffset, 0xffff00);
		wScissor -= 30;
		hScissor -= 30;
		Ft_App_WrCoCmd_Buffer(phost, SCISSOR_SIZE(wScissor, hScissor));
		xOffset += 15;
		yOffset += 15;
		Ft_App_WrCoCmd_Buffer(phost, SCISSOR_XY(xOffset, yOffset)); //clip the display
		Ft_Gpu_CoCmd_Gradient(phost, xOffset, yOffset, 0x800000, xOffset,
		...
		yOffset += 15;
		Ft_App_WrCoCmd_Buffer(phost, SCISSOR_XY(xOffset, yOffset)); //clip the display
		Ft_Gpu_CoCmd_Gradient(phost, xOffset, yOffset, 0x606060,
				(xOffset + wScissor), (yOffset + hScissor), 0x404080);
...
#endif
		Ft_Gpu_CoCmd_Text(phost, xOffset, yOffset, 26, OPT_CENTER,
				"Horizontal grad"); //text info
		xOffset += wScissor + 10;
		Ft_Gpu_CoCmd_Text(phost, xOffset, yOffset, 26, OPT_CENTER,
				"Vertical grad"); //text info
		xOffset += wScissor + 10;
		Ft_Gpu_CoCmd_Text(phost, xOffset, yOffset, 26, OPT_CENTER,
				"Diagonal grad"); //text info
...
ft_void_t SAMAPP_CoPro_Widget_Keys_Interactive() {
	...
	/*************************************************************************/
	{
		...
		ft_int16_t TextFont = 29, ButtonW = 30, ButtonH = 30, yBtnDst = 5,
				yOffset, xOffset;
#define SAMAPP_COPRO_WIDGET_KEYS_INTERACTIVE_TEXTSIZE (512)
		ft_char8_t DispText[SAMAPP_COPRO_WIDGET_KEYS_INTERACTIVE_TEXTSIZE],
				CurrChar = '|';
		...
		while (loopflag--) {
			...
			Ft_Gpu_CoCmd_GradColor(phost, 0x00ff00);
			Ft_Gpu_CoCmd_Keys(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
					TextFont, (OPT_CENTER | CurrTag), "qwertyuiop");
			Ft_Gpu_CoCmd_GradColor(phost, 0x00ffff);
			yOffset += ButtonH + yBtnDst;
			Ft_Gpu_CoCmd_Keys(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
					TextFont, (OPT_CENTER | CurrTag), "asdfghjkl");
			Ft_Gpu_CoCmd_GradColor(phost, 0xffff00);
			yOffset += ButtonH + yBtnDst;
			Ft_Gpu_CoCmd_Keys(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
					TextFont, (OPT_CENTER | CurrTag), "zxcvbnm"); //hilight button z
			yOffset += ButtonH + yBtnDst;
			Ft_App_WrCoCmd_Buffer(phost, TAG(' '));
			if (' ' == CurrTag) {
				Ft_Gpu_CoCmd_Button(phost, yBtnDst, yOffset, 10 * ButtonW,
						ButtonH, TextFont, OPT_CENTER | OPT_FLAT, " "); //mandatory to give '\0' at the end to make sure coprocessor understands the string end
			} else {
				Ft_Gpu_CoCmd_Button(phost, yBtnDst, yOffset, 10 * ButtonW,
						ButtonH, TextFont, OPT_CENTER, " "); //mandatory to give '\0' at the end to make sure coprocessor understands the string end
			}
			yOffset = ButtonW * 2 + 10;
			Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW,
					ButtonH, TextFont, (0 | CurrTag), "789");
			yOffset += ButtonH + yBtnDst;
			Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW,
					ButtonH, TextFont, (0 | CurrTag), "456");
			yOffset += ButtonH + yBtnDst;
			Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW,
					ButtonH, TextFont, (0 | CurrTag), "123");
			yOffset += ButtonH + yBtnDst;
			Ft_App_WrCoCmd_Buffer(phost, COLOR_A(255));
			Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW,
					ButtonH, TextFont, (0 | CurrTag), "0."); //hilight button 0
			Ft_App_WrCoCmd_Buffer(phost, DISPLAY());
			Ft_Gpu_CoCmd_Swap(phost);
...
ft_void_t SAMAPP_CoPro_Widget_Keys() {
	...
	/*************************************************************************/
	{
		...
		Ft_Gpu_CoCmd_FgColor(phost, 0x404080);
		Ft_Gpu_CoCmd_Keys(phost, 10, 10, 4 * ButtonW, 30, TextFont, OPT_FLAT,
				"ABCD");
		...
		Ft_Gpu_CoCmd_Keys(phost, xOffset, 10, 4 * ButtonW, 30, TextFont, 0,
				"ABCD");
		...
		xOffset += 4 * ButtonW + 20;
		Ft_Gpu_CoCmd_Keys(phost, xOffset, 10, (FT_DispWidth - 230), 30,
				TextFont,
				OPT_CENTER, "ABCD");
		...
		Ft_Gpu_CoCmd_GradColor(phost, 0x00ff00);
		Ft_Gpu_CoCmd_Keys(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
				TextFont, OPT_CENTER, "qwertyuiop");
		Ft_Gpu_CoCmd_GradColor(phost, 0x00ffff);
		yOffset += ButtonH + yBtnDst;
		Ft_Gpu_CoCmd_Keys(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
				TextFont, OPT_CENTER, "asdfghjkl");
		Ft_Gpu_CoCmd_GradColor(phost, 0xffff00);
		yOffset += ButtonH + yBtnDst;
		Ft_Gpu_CoCmd_Keys(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
				TextFont, (OPT_CENTER | 'z'), "zxcvbnm"); //hilight button z
		yOffset += ButtonH + yBtnDst;
		Ft_Gpu_CoCmd_Button(phost, yBtnDst, yOffset, 10 * ButtonW, ButtonH,
				TextFont, OPT_CENTER, " "); //mandatory to give '\0' at the end to make sure coprocessor understands the string end
		yOffset = 80 + 10;
		Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW, ButtonH,
				TextFont, 0, "789");
		yOffset += ButtonH + yBtnDst;
		Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW, ButtonH,
				TextFont, 0, "456");
		...
		Ft_App_WrCoCmd_Buffer(phost, COLOR_A(255));
		Ft_Gpu_CoCmd_Keys(phost, 11 * ButtonW, yOffset, 3 * ButtonW, ButtonH,
				TextFont, (0 | '0'), "0."); //hilight button 0
		Ft_App_WrCoCmd_Buffer(phost, DISPLAY());
		Ft_Gpu_CoCmd_Swap(phost);
...
ft_void_t SAMAPP_CoPro_Widget_Progressbar() {
	...
	/*************************************************************************/
	{
		ft_int16_t xOffset, yOffset, yDist = FT_DispWidth / 12, ySz =
				FT_DispWidth / 24;
...
ft_void_t SAMAPP_CoPro_Widget_Scroll() {
	...
	/*************************************************************************/
	{
		ft_int16_t xOffset, yOffset, xDist = FT_DispWidth / 12, yDist =
				FT_DispWidth / 12, wSz;
...
ft_void_t SAMAPP_CoPro_Widget_Toggle() {
	...
	/*************************************************************************/
	{
		...
		Ft_Gpu_CoCmd_Toggle(phost, 40, 10, 30, 27, OPT_FLAT, 0.5 * 65535,
				"no" "\xff" "yes"); //circle radius will be extended on both the horizontal sides
		Ft_App_WrCoCmd_Buffer(phost, COLOR_A(255));
		Ft_Gpu_CoCmd_Text(phost, 40, 40, 26, 0, "Flat effect"); //text info
		/* Draw toggle bar with 3d effect */
		Ft_Gpu_CoCmd_FgColor(phost, 0x00ff00);
		Ft_Gpu_CoCmd_BgColor(phost, 0x800000);
		Ft_Gpu_CoCmd_Toggle(phost, 140, 10, 30, 27, 0, 1 * 65535,
				"stop" "\xff" "run");
		Ft_App_WrCoCmd_Buffer(phost, COLOR_A(255));
		...
		Ft_Gpu_CoCmd_Toggle(phost, xOffset, yOffset, 30, 27, 0, 0 * 65535,
				"-ve" "\xff" "+ve");
		Ft_Gpu_CoCmd_FgColor(phost, 0x0b0721);
		Ft_Gpu_CoCmd_BgColor(phost, 0x000080);
		yOffset += yDist;
		Ft_Gpu_CoCmd_Toggle(phost, xOffset, yOffset, 30, 27, 0, 0.25 * 65535,
				"zero" "\xff" "one");
		Ft_Gpu_CoCmd_BgColor(phost, 0xffff00);
		Ft_Gpu_CoCmd_FgColor(phost, 0x575e1b);
		Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(0, 0, 0));
		yOffset += yDist;
		Ft_Gpu_CoCmd_Toggle(phost, xOffset, yOffset, 30, 27, 0, 0.5 * 65535,
				"exit" "\xff" "init");
		Ft_Gpu_CoCmd_BgColor(phost, 0x808080);
		Ft_Gpu_CoCmd_FgColor(phost, 0x333234);
		Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(0xff, 0xff, 0xff));
		yOffset += yDist;
		Ft_Gpu_CoCmd_Toggle(phost, xOffset, yOffset, 30, 27, 0, 0.75 * 65535,
				"off" "\xff" "on");
...
ft_void_t SAMAPP_CoPro_Widget_Spinner() {
	...
	/*************************************************************************/
	{
		...
		Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
		Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(0xff, 0xff, 0xff));
		Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 20, 27, OPT_CENTER,
				"Spinner circle");
		Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80, 27, OPT_CENTER,
				"Please Wait ...");
		Ft_Gpu_CoCmd_Spinner(phost, (FT_DispWidth / 2), (FT_DispHeight / 2), 0,
				1); //style 0 and scale 0
		...
		Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80, 27, OPT_CENTER,
				"Please Wait ...");
		Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(0x00, 0x00, 0x80));
		Ft_Gpu_CoCmd_Spinner(phost, (FT_DispWidth / 2), (FT_DispHeight / 2), 1,
				1); //style 1 and scale 1
		...
		Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80, 27, OPT_CENTER,
				"Please Wait ...");
		Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(0x80, 0x00, 0x00));
		Ft_Gpu_CoCmd_Spinner(phost, (FT_DispWidth / 2),
				((FT_DispHeight / 2) + 20), 2, 1); //style 2 scale 1
	...
	/*************************************************************************/
	{
		...
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_LAYOUT(SAMAPP_Bitmap_RawData_Header[0].Format,
						...
						SAMAPP_Bitmap_RawData_Header[0].Height));
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_SIZE(BILINEAR, BORDER, BORDER,
						SAMAPP_Bitmap_RawData_Header[0].Width * 3,
...
// color vertices
ft_uint16_t colors[6][3] = { 255, 0, 0, 255, 0, 150, 0, 255, 0, 110, 120, 110,
		0, 0, 255, 128, 128, 0 };
...
for (z = 0; z < 6; z++) {
	...
	Ft_App_WrDlCmd_Buffer(phost,
			COLOR_RGB(colors[z][0], colors[z][1], colors[z][2]));
	Ft_App_WrDlCmd_Buffer(phost, BEGIN(RECTS));
	Ft_App_WrDlCmd_Buffer(phost, VERTEX2II(xoffset, 0, 0, 0));
	Ft_App_WrDlCmd_Buffer(phost,
			VERTEX2II(xoffset + CubeEdgeSz * 2, yoffset + CubeEdgeSz * 2, 0,
					0));
	Ft_App_WrDlCmd_Buffer(phost, END());
}
Ft_App_WrDlCmd_Buffer(phost, DISPLAY());
...
ft_int32_t asmooth, loopflag = 1, dispr = (FT_DispWidth - 10), displ = 10,
		dispa = 10, dispb = (FT_DispHeight - 10), xflag = 1, yflag = 1;
...
while (loopflag-- > 0) {
	...
	for (i = 0; i <= ((dispr - displ) / gridsize); i++) {
		Ft_App_WrDlCmd_Buffer(phost,
				VERTEX2F((displ + i * gridsize) * 16, dispa * 16));
		Ft_App_WrDlCmd_Buffer(phost,
				VERTEX2F((displ + i * gridsize) * 16, dispb * 16));
	}
	for (i = 0; i <= ((dispb - dispa) / gridsize); i++) {
		Ft_App_WrDlCmd_Buffer(phost,
				VERTEX2F(displ * 16, (dispa + i * gridsize) * 16));
		Ft_App_WrDlCmd_Buffer(phost,
				VERTEX2F(dispr * 16, (dispa + i * gridsize) * 16));
	}
	Ft_App_WrDlCmd_Buffer(phost, END());
	Ft_App_WrDlCmd_Buffer(phost, COLOR_MASK(0, 0, 0, 0)); //mask all the colors
	...
	for (i = 1; i <= numpoints; i++) {
		...
		asmooth = (ft_int16_t) smoothlerp(
				(float) ((float) (asize) / (2 * (float) rball)), 0,
				2 * (float) rball);
		...
		if (asmooth > rball) {
			...
			aradius = (rball * rball + tempsmooth * tempsmooth)
					/ (2 * tempsmooth);
	...
	for (i = 1; i <= numlines; i++) {
		...
		Ft_App_WrDlCmd_Buffer(phost,
				VERTEX2F((xball - rball) * 16, (yball - rball) * 16));
		Ft_App_WrDlCmd_Buffer(phost,
				VERTEX2F((xball + rball) * 16, (yball - rball) * 16));
	}
	Ft_App_WrDlCmd_Buffer(phost, END());
...
while (LoopFlag--) {
	...
	Ft_Gpu_Hal_Dec2Ascii(StringArray, (ft_int32_t) yvalue);
	strcat(StringArray, ")");
	Ft_Gpu_CoCmd_Text(phost, FT_DispWidth / 2, 110, 26, OPT_CENTER,
			StringArray);
	...
	Ft_Gpu_CoCmd_Text(phost, FT_DispWidth / 2, 130, 26, OPT_CENTER,
			StringArray);
	...
	strcat(StringArray, ")");
	Ft_Gpu_CoCmd_Text(phost, FT_DispWidth / 2, 170, 26, OPT_CENTER,
			StringArray);
	tagval = ReadWord;
	...
	Ft_Gpu_CoCmd_Button(phost, (FT_DispWidth / 2) - (wbutton / 2),
			(FT_DispHeight * 3 / 4) - (hbutton / 2), wbutton, hbutton, 26,
			tagoption, "Tag13");
...
#ifdef SAMAPP_ENABLE_APIS_SET4
FT_PROGMEM ft_prog_uchar8_t SAMAPP_Snd_Array[5 * 58] =
...
FT_PROGMEM ft_prog_uchar8_t SAMAPP_Snd_TagArray[58] = { 0x63, 0x1, 0x2, 0x3,
	0x4, 0x5, 0x6, 0x7, 0x8, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
	0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x23, 0x2a, 0x30, 0x31,
	0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x40, 0x41, 0x42, 0x43,
	0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55,
	0x56, 0x57, 0x58, 0x60, 0x61 };

/* API to demonstrate the usage of sound engine of FT800 */
ft_void_t SAMAPP_Sound() {
ft_int32_t LoopFlag = 0, wbutton, hbutton, tagval, tagvalsnd = -1, numbtnrow,
		numbtncol, i, j, prevtag = -1;
ft_uint32_t freqtrack = 0, currfreq = 0, prevcurrfreq;
FT_PROGMEM ft_prog_uchar8_t
*pString;
FT_PROGMEM ft_prog_uchar8_t
*pTagArray;
...
while (LoopFlag--) {
	...
	if ((tagval > 0)) {
		...
		if ((prevtag != tagval) || (prevcurrfreq != currfreq)) {
			...
			Ft_Gpu_Hal_Wr16(phost, REG_SOUND,
					(((currfreq + 21) << 8) | tagvalsnd));
			Ft_Gpu_Hal_Wr8(phost, REG_PLAY, 1);
		}
		if (0 == tagvalsnd)
			tagvalsnd = 99;
	...
	for (i = 0; i < numbtnrow; i++) {
		for (j = 0; j < numbtncol; j++) {
			...
			strcpy(StringArray,pString);
#endif                                
			Ft_Gpu_CoCmd_Text(phost, (wbutton / 2) + j * wbutton,
					(hbutton / 2) + hbutton * i, 26, OPT_CENTER, StringArray);
	...
	Ft_App_WrCoCmd_Buffer(phost, TAG_MASK(0));
	Ft_Gpu_CoCmd_Text(phost, FT_DispWidth - 20, 10, 26, OPT_CENTER,
			StringArray);
	Ft_App_WrCoCmd_Buffer(phost, TAG_MASK(1));
	Ft_App_WrCoCmd_Buffer(phost, TAG(100));
	Ft_Gpu_CoCmd_Slider(phost, FT_DispWidth - 15, 20, 8, (FT_DispHeight - 40),
			0, currfreq, 88);
...
#if defined(FT900_PLATFORM)
TI2CMBasicConfig I2CMConfig;
I2CM_STATUS i2cmStatus;
unsigned char codecControl[2]; // codecControl[0]: codec's register address, codecControl[1]: codec's register data
...
Ft_Gpu_HostCommand(phost,FT_GPU_SLEEP_M);
Ft_Gpu_Hal_Sleep(50);
...
/* Switch PLL output to 48MHz for FT80x or 60MHz for FT81x */
...
FT_DispDither = 1;
#endif
#ifdef SAMAPP_DISPLAY_HVGA_PORTRAIT
...
ft_delay(300);
#ifdef FT_811_ENABLE
Ft_Gpu_Hal_Wr16(phost, REG_CYA_TOUCH,
		(Ft_Gpu_Hal_Rd16(phost, REG_CYA_TOUCH) & 0x7fff)); //capacitive touch
#endif
#ifdef FT_810_ENABLE
...
#if 0//example code to change to quad spi for ft81x chip
...
#ifdef ENABLE_SPI_SINGLE
SPIMConfig.format = DQSPI_TRANSFER_STD;			// Single data transfer
printf("Single channel SPI Configured \n");
#endif
...
#endif
SPIMConfig.ratio = DQSPI_CR8;           			// CLK/8
SPIMConfig.polarity = DQSPI_SCK_CPOL_0;// SCK polarity 0
SPIMConfig.phase = DQSPI_SCK_CPHA_0;// SCK phase 0
SPIMConfig.slaveAutoSS = DQSPI_AUTO_SS_DISABLED;// Disable SS auto assertion
SPIMConfig.fifoEnable = DQSPI_FIFO_ENABLED;// Enable FIFO
SPIMConfig.fifoMode = DQSPI_FIFO_DEPTH_64;// FIFO depth 64 bytes
SPIMConfig.fifoTO = DQSPI_FIFO_TIMEOUT_DISABLED;// FIFO timeout interrupt disabled
SPIMConfig.fifoRxTrigger = DQSPI_FR_FIRST;// FIFO receive trigger at 1st byte
SPIMConfig.txEmpty = DQSPI_TX_EMPTY_DISABLED;// Disable transmitter empty interrupt
SPIMConfig.modeDMA = True;// No DMA
dqspiInit(SPIM, &SPIMConfig);
// Reset XMIT and RCVR FIFO on both master and slave
dqspiResetFIFO(SPIM, DQSPI_RX_TX_FIFO);
#endif
...
#if defined(FT900_PLATFORM)
// Initialize I2CM HW, swop mode enabled
I2CMSysInit(True);
// Configure I2CM
i2cmSoftReset();
I2CMConfig.sysClk = 100000000;
I2CMConfig.clk = 100000;
I2CMConfig.speedMode = I2CM_NORMAL_SPEED;
i2cmInit(&I2CMConfig);
// Set audio codec's address
i2cmSetSlaveAddr(0x1A);

codecControl[0] = 0x1E;    // reset codec
codecControl[1] = 0x00;    // reset
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

if (i2cmStatus != I2CM_OK) {
	printf("error in codec reset!  %d \n", i2cmStatus);
}
codecControl[0] = 0x0C;    // power reduction register
codecControl[1] = 0x42;    // power down line input, microphone, ADC, CLKOUT
// codecControl[1] = 0x42; // power down microphone, ADC, CLKOUT
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

#if 0
codecControl[0] = 0x0E;    // digital data format
codecControl[1] = 0x02;// I2S slave mode, 16-bit data
// codecControl[1] = 0x42; // I2S master mode, 16-bit data
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

if (i2cmStatus != I2CM_OK)
{
	printf("Cannot write to codec's digital data format register!\n");
}
#endif

codecControl[0] = 0x00;    // left in setup register
codecControl[1] = 0x17;
// codecControl[1] = 0x1F; // set max volume for left line-in
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

if (i2cmStatus != I2CM_OK) {
	printf("Cannot write to codec's left in setup register!\n");
}

codecControl[0] = 0x02;    // right in setup register
codecControl[1] = 0x17;
// codecControl[1] = 0x1F;
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

if (i2cmStatus != I2CM_OK) {
	printf("Cannot write to codec's right in setup register!\n");
}

codecControl[0] = 0x04;    // left headphone out register
codecControl[1] = 0xFF;
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

if (i2cmStatus != I2CM_OK) {
	printf("Cannot write to codec's left headphone out register!\n");
}

codecControl[0] = 0x06;    // right headphone out register
codecControl[1] = 0xFF;
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

if (i2cmStatus != I2CM_OK) {
	printf("Cannot write to codec's right headphone out register!\n");
}

// Now start audio codec
codecControl[0] = 0x00;    // linein volume
codecControl[1] = 0x1F; // max volume
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));

// Now start audio codec
codecControl[0] = 0x12;    // codec enable register
codecControl[1] = 0x01; // activate codec
i2cmStatus = i2cmSendBytes(codecControl, sizeof(codecControl));
if (i2cmStatus != I2CM_OK) {

	printf("Cannot start audio codec!\n");
...
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G + 1000, SAMApp_ChineseFont_MetricBlock,
SAMAPP_ChineseFont_Metric_SIZE);
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G + 1000 + SAMAPP_ChineseFont_Metric_SIZE,
		SAMApp_ChineseFont_FontBmpData,
		SAMAPP_ChineseFont_BitmapData_SIZE);
...
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 20, 27, OPT_CENTER,
		"FangSong Font L8 Traditional Chinese");
...
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80, 7, OPT_CENTER,
		"\x01\x02\x03\x04\x05");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80 + 34, 7, OPT_CENTER,
		"\x06\x07\x08\x09\x0A");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80 + 34 + 34, 7, OPT_CENTER,
		"\x0B\x0C\x0D\x0E\x0F");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 80 + 34 + 34 + 34, 7, OPT_CENTER,
		"\x10\x11\x12\x13\x14");

Ft_Gpu_CoCmd_Button(phost, (FT_DispWidth / 2), 80 + 34 + 34 + 34 + 34, 80, 34,
		7, 0, "\x15\x16");
...
ft_void_t BouncingSquaresCall(ft_int16_t BRx, ft_int16_t BRy,
	ft_int16_t MovingRy, ft_uint8_t SqNumber) {
...
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2F((MovingRx + 45) * 16, (MovingRy + 5) * 16));
}

ft_int16_t MovingRect(ft_int16_t BRy, ft_int16_t MovingRy,
	ft_uint8_t EndPtReach) {
if (MovingRy <= 0) {
	EndPtReach = 0;
	MovingRy = 1;
}
...
if (context->RectNo[Arrayno] == 1) {
	...
	MovingRy1 = MovingRect(context->BRy[Arr], context->My[Arr],
			context->E[Arr]);
...
ft_void_t CheckTouch(BouncingSquares_Context *context, ft_int16_t Tx1,
	ft_int32_t val1) {
...
for (k = 0; k < 800; k++)
	...
	/*fifth  touch*/
	val[4] = ((ft_uint32_t) Ft_Gpu_Hal_Rd16(phost, REG_CTOUCH_TOUCH4_X) << 16L)
			| (Ft_Gpu_Hal_Rd16(phost, REG_CTOUCH_TOUCH4_Y) & 0xffffL);
...
ft_uint16_t ConcentricCircles(float C1, ft_uint16_t R, ft_uint16_t G,
	ft_uint16_t B) {
...
ft_uint8_t CheckCircleTouchCood(BouncingCircles_Context *context,
	ft_int32_t val, ft_uint8_t TouchNum, ft_uint8_t i) {
double CX = 0;
double CY = 0;
...
if ((val >> 16) != -32768) {
	...
	if (AllClear == 1) {
		if (context->TN[TouchNum].F[i] != 1) {
			...
			if ((CX > (context->C1X[i] - 15))
					&& (CX < (context->C1X[i] + 15))) {
				if ((CY > (context->C1Y[i] - 30))
						&& (CY < context->C1Y[i] + 30)) {
...
ft_uint16_t CirclePlot(BouncingCircles_Context *context, ft_uint16_t X,
	ft_uint16_t Y, ft_uint8_t Val) {
...
ft_void_t StoreTouch(BouncingCircles_Context *context, ft_int32_t Touchval,
	ft_uint8_t TouchNo) {
...
for (k = 0; k < 800; k++)
	...
	Touchval[4] = ((ft_int32_t) Ft_Gpu_Hal_Rd16(phost, REG_CTOUCH_TOUCH4_X)
			<< 16) | (Ft_Gpu_Hal_Rd16(phost, REG_CTOUCH_TOUCH4_Y));
	...
	for (i = 0; i < NO_OF_CIRCLE; i++) {
		context.Tsq1[i] = CirclePlot(&context, context.C1X[i], context.C1Y[i],
				i);
	}
...
for(k=0;k<350;k++)
	...
	val[2] = Ft_Gpu_Hal_Rd32(phost, REG_CTOUCH_TOUCH2_XY);
	val[3] = Ft_Gpu_Hal_Rd32(phost, REG_CTOUCH_TOUCH3_XY);
	val[4] = (((ft_int32_t) Ft_Gpu_Hal_Rd16(phost, REG_CTOUCH_TOUCH4_X) << 16)
			| (Ft_Gpu_Hal_Rd16(phost, REG_CTOUCH_TOUCH4_Y) & 0xffff));
...
ft_void_t PointsCalc(MovingPoints_Context *context, ft_uint8_t TouchNo,
	ft_int16_t *X, ft_int16_t *Y, ft_uint8_t *t) {
...
for (k = 0; k < NO_OF_POINTS * 4L; k++) {
	{
		...
		if (context->X[k] != -32768) {
			tempDeltaX = linear(context->X[k], context->SmallX[pointset],
					context->t[k], NO_OF_POINTS);
			tempDeltaY = linear(context->Y[k], context->SmallY, context->t[k],
			NO_OF_POINTS);
			Ft_App_WrCoCmd_Buffer(phost,
					VERTEX2F(tempDeltaX * 16L, tempDeltaY * 16L));
		}
		t[k]++;
	}
}
...
FIL InfSrc;
ft_int32_t blocklen, filesize, bufSz = 2048, g_random_buffer[bufSz];
char tempName[14] = "0:\\";
...
if (fResult == FR_OK) {
	...
	while (filesize > 0) {
		fResult = f_read(&InfSrc, g_random_buffer, bufSz * 4, &blocklen);// read a chunk of src file
		filesize -= blocklen;
		Ft_Gpu_Hal_WrMem(phost, ramOffset, g_random_buffer, blocklen); /* copy data continuously into ram */
		ramOffset += blocklen;
	}
	f_close(&InfSrc);
} else {
...
#if defined(FT_81X_ENABLE) || defined(SAMAPP_DISPLAY_WVGA)



ft_void_t SAMAPP_Load_Data_To_CmdBuff(ft_char8_t *filename){
#if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
	ft_uint32_t FileLen;
	ft_uchar8_t *pbuff;
...
ft_int32_t SAMAPP_load_inflated_image(ft_char8_t *filename, ft_uint32_t address) {
...
#elif defined(FT900_PLATFORM)
FIL InfSrc;
ft_int32_t blocklen, filesize, bufSz = 2048, g_random_buffer[bufSz], freeSpace;
char tempName[14] = "0:\\";
strcat(tempName, filename);
fResult = f_open(&InfSrc, tempName, FA_READ | FA_OPEN_EXISTING);
if (fResult == FR_OK) {
	//printf("success in opening file %s \n",filename);
	Ft_Gpu_CoCmd_Inflate(phost, address);
	//Ft_Gpu_Hal_WrCmd32(phost,  CMD_LOADIMAGE);
	//Ft_Gpu_Hal_WrCmd32(phost,  address);//destination address of jpg decode
...
FILE *pFile;
    ...
    Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
#if defined(FT900_PLATFORM)
    fResult = f_open(&CurFile, "0:\\chickens-4.avi", FA_READ | FA_OPEN_EXISTING);
	if (fResult == FR_OK)
	{
		printf("\nFopen success!\n");
...
#elif defined(FT900_PLATFORM)
				fResult = f_read(&CurFile,g_scratch,tempchunk,&bytesread);
#endif
...
#elif defined(FT900_PLATFORM)
				Ft_Gpu_Hal_StartTransfer(phost,FT_GPU_WRITE,stFifo.fifo_buff + stFifo.fifo_wp);
				dqspiWritePool(SPIM,g_scratch, tempchunk,DQSPI_M_DISCARD_INCOMING_DATA);
...
ft_uint32_t dlList = RAM_DL;
#if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
ft_uchar8_t *fbuff, *buffIndex;
ft_uchar8_t pbuff[BUFSIZE];
FILE *pFile = fopen("..\\..\\..\\Test\\Tomato.png","rb");
//FILE *pFile = fopen("..\\..\\..\\Test\\EiffelTower_800_480.jpg","rb");
...
fbuff = malloc(FileLen);
buffIndex = fbuff;
fread(fbuff,1,FileLen,pFile);
//Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G,(ft_uchar8_t *)pbuff,  FileLen);
fclose(pFile);

//show stock image by CMD_LOADIMAGE
dlPTR=0;
Ft_Gpu_CoCmd_Dlstart(phost);
//Ft_App_WrCoCmd_Buffer(phost,CLEAR_COLOR_RGB(255,255,255) );
Ft_App_WrCoCmd_Buffer(phost,CLEAR(1,1,1) );
...
{
	/* download the data into the command buffer by 2kb one shot */
	ft_uint16_t blocklen = (dlPTR+BUFSIZE)>FileLen?(FileLen-dlPTR):BUFSIZE;

	/* copy data continuously into command memory */
	Ft_Gpu_Hal_WrCmdBuf(phost,buffIndex+dlPTR, blocklen); //4 byte alignment is taken care by this api
	dlPTR+=blocklen;
	SAMAPP_ENABLE_DELAY_VALUE(500);
}
#elif defined(FT900_PLATFORM) || defined(ARDUINO_PLATFORM)
Ft_Gpu_CoCmd_Dlstart(phost);
Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
Ft_Gpu_Hal_WrCmd32(phost, CMD_LOADIMAGE);
Ft_Gpu_Hal_WrCmd32(phost, RAM_G); //destination address for the decoded data
Ft_Gpu_Hal_WrCmd32(phost, 0);
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_128_128_PNG_Hex_Dump, 7934);
//SAMAPP_load_inflated_image("tp565.png",RAM_G);
#endif

Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 50, 31, OPT_CENTER, "PNG images can now be loaded directly.");
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));

/*
//the decoded data is in the ram but the default display list is not inserted automatically.
 Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(0));
 Ft_App_WrCoCmd_Buffer(phost, BITMAP_LAYOUT(ARGB4,bitmapWidth*2,bitmapHeight));
 Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE(NEAREST,BORDER,BORDER,bitmapWidth,bitmapHeight));
 */
...
//Ft_App_WrCoCmd_Buffer(phost, VERTEX2F((FT_DispWidth / 2 - 400) * 16, (FT_DispHeight / 2 - 240) * 16));
Ft_App_WrCoCmd_Buffer(phost, END());
...
//printf("Total cmd: %d\n",Ft_Gpu_Hal_Rd32(phost,REG_CMD_DL));

/*
 for(;dlList<(RAM_DL+Ft_Gpu_Hal_Rd32(phost,REG_CMD_DL));dlList+=4)
 printf("%x\n",Ft_Gpu_Hal_Rd32(phost,dlList));
 while(1);  //2044865
 */
...
/*
 ...
 Ft_Gpu_CoCmd_Text(phost,(FT_DispWidth/2), 30, 31, OPT_CENTER, "Bitmap upto 2048pix x 2048pix");
 Ft_Gpu_CoCmd_Text(phost,(FT_DispWidth/2), 65, 31, OPT_CENTER, "and");
 Ft_Gpu_CoCmd_Text(phost,(FT_DispWidth/2), 105, 31, OPT_CENTER, "Auto scale to full screen size.");
 ...
 Ft_App_WrCoCmd_Buffer(phost,CLEAR(1,1,1) );
 #if defined(MSVC_PLATFORM)
 //Display the bitmap in full screen mode
 dlPTR=0;
 Ft_Gpu_Hal_WrCmd32(phost,  CMD_LOADIMAGE);
 Ft_Gpu_Hal_WrCmd32(phost,  RAM_G);//destination address of jpg decode
 Ft_Gpu_Hal_WrCmd32(phost,  OPT_FULLSCREEN);//output format of the bitmap, supported options are OPT_MONO-bitmap to be monochrome, OPT_FULLSCREEN-attempt to scale to full screen size, and OPT_MEDIAFIFO-use media fifo for image data.

 while(dlPTR != FileLen)
 {
 ft_uint16_t blocklen = (dlPTR+BUFSIZE)>FileLen?(FileLen-dlPTR):BUFSIZE;

 Ft_Gpu_Hal_WrCmdBuf(phost,fbuff+dlPTR, blocklen);//4 byte alignment is taken care by this api
 dlPTR+=blocklen;
 }
 #elif defined(FT900_PLATFORM) || defined(ARDUINO_PLATFORM)
 Ft_Gpu_Hal_WrCmd32(phost,  CMD_LOADIMAGE);
 Ft_Gpu_Hal_WrCmd32(phost,  RAM_G);//destination address if inflate
 Ft_Gpu_Hal_WrCmd32(phost,  OPT_FULLSCREEN);
 Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_128_128_PNG_Hex_Dump, 7934);
 #endif
...
ft_uint32_t dlList = RAM_DL, i,j;

/*
FIL InfSrc;
ft_int32_t blocklen, filesize, bufSz = 2048, bytesWritten;
ft_uint8_t g_random_buffer[1024];
*/

 Ft_Gpu_CoCmd_Dlstart(phost);
 //Ft_App_WrCoCmd_Buffer(phost, CLEAR_COLOR_RGB(255, 255, 255));
 //Ft_App_WrCoCmd_Buffer(phost, CLEAR_COLOR_RGB(0, 80, 80));
 Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
	Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(255, 255, 255));
	Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 50, 31, OPT_CENTER, "Paletted");
 #if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
	/*
 paletteTbSz = SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\t_lut.raw", RAM_G);
 paletteIdxSz = SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\t_index.raw", 1024);
 */
paletteTbSz = SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\t5_lut.raw", RAM_G);
 paletteIdxSz = SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\t5_index.raw", 1024);
#elif defined(FT900_PLATFORM) || defined(ARDUINO_PLATFORM)
paletteTbSz = SAMAPP_LoadRawFromFile("tt.raw", RAM_G);
 paletteIdxSz = SAMAPP_LoadRawFromFile("ti.raw", 1024);

 paletteTbSz = SAMAPP_LoadRawFromFile("t2t.raw", 3000);
 paletteIdxSz = SAMAPP_LoadRawFromFile("t2i.raw", 5000);
#endif

  Ft_App_WrCoCmd_Buffer(phost, PALETTE_SOURCE(RAM_G));  //specify the palette table address
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(1024));//The palette index table is loaded after the palette table
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(PALETTED565,bitmapStride,bitmapHeight));//The stride is 800 because paletted format is 1 byte per pixel and height is 480
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST,BORDER,BORDER,bitmapWidth,bitmapHeight));//bitmap size is 800 x 480
 Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center

 /*
 Ft_App_WrCoCmd_Buffer(phost, PALETTE_SOURCE(RAM_G));  //specify the palette table address
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(1024));//The palette index table is loaded after the palette table
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(PALETTED8,40,40));//The stride is 800 because paletted format is 1 byte per pixel and height is 480
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST,BORDER,BORDER,40,40));//bitmap size is 800 x 480
 Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center

  Ft_App_WrCoCmd_Buffer(phost, PALETTE_SOURCE(3000));  //specify the palette table address
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(5000));//The palette index table is loaded after the palette table
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(PALETTED8,40,40));//The stride is 800 because paletted format is 1 byte per pixel and height is 480
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST,BORDER,BORDER,40,40));//bitmap size is 800 x 480
 Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2 + 40) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center

 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(1024));//The palette index table is loaded after the palette table
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L8,40,40));//The stride is 800 because paletted format is 1 byte per pixel and height is 480
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST,BORDER,BORDER,40,40));//bitmap size is 800 x 480
 Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2 - 40) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center
 */
 Ft_App_WrCoCmd_Buffer(phost,DISPLAY());
...
fResult = f_open(&InfSrc, "0:\RD.raw", FA_WRITE | FA_OPEN_ALWAYS);
if (fResult != FR_OK) {
	printf("error in opening file %s \n");
	return;
	f_close(&InfSrc);
 		...
 		Ft_Gpu_CoCmd_Snapshot2(phost, 0x20, 100*1024, 0, 0, 800, 120);

		Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
		Ft_Gpu_Hal_Sleep(1000); //timeout for snapshot to be performed by coprocessor

		for(i=100*1024; i<(384000+100*1024);i+=1024){
			Ft_Gpu_Hal_RdMem(phost, i,  g_random_buffer, 1024);
			f_write(&InfSrc, g_random_buffer, 1024, &bytesWritten);
						printf(".");
			if(bytesWritten != 1024)
				printf("Error encountered\n");
		}
		printf("Finished 1st chunk\n");

		Ft_Gpu_CoCmd_Snapshot2(phost, 0x20, 100*1024, 0, 120, 800, 120);
		Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
		Ft_Gpu_Hal_Sleep(1000); //timeout for snapshot to be performed by coprocessor
		...
		printf("Finished 2nd chunk\n");

		 		Ft_Gpu_CoCmd_Snapshot2(phost, 0x20, 100*1024, 0, 240, 800, 120);
		Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
		Ft_Gpu_Hal_Sleep(1000); //timeout for snapshot to be performed by coprocessor
		...
		printf("Finished 3rd chunk\n");

				 		Ft_Gpu_CoCmd_Snapshot2(phost, 0x20, 100*1024, 0, 360, 800, 120);
		Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
		Ft_Gpu_Hal_Sleep(1000); //timeout for snapshot to be performed by coprocessor
...
Ft_Gpu_Hal_WrCmd32(phost, CMD_INFLATE);
Ft_Gpu_Hal_WrCmd32(phost, RAM_G);	//destination address if inflate
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_Palette_Table_Bin, 954);
Ft_Gpu_Hal_WrCmd32(phost, CMD_INFLATE);
Ft_Gpu_Hal_WrCmd32(phost, 1024);//destination address if inflate
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_Palette_Indexes_Bin, 6801);
*/

/*
 paletteTbSz = SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_128_128_lut.raw", RAM_G);
 paletteIdxSz = SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_128_128_index.raw", 1024);
 */
...
/*
 ...
 Ft_Gpu_Hal_WrCmd32(phost, CMD_INFLATE);
 Ft_Gpu_Hal_WrCmd32(phost, 1024);	//destination address if inflate
 Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_Palette_Indexes_Bin,
 6801);
 */
...
Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(255, 255, 255));
//Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 50, 31, OPT_CENTER, "Paletted Bitmap: 16KB.");
//Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 95, 31, OPT_CENTER, "Original: 32KB.");

Ft_Gpu_Hal_WrCmd32(phost, CMD_LOADIMAGE);
Ft_Gpu_Hal_WrCmd32(phost, 0);	//destination address if inflate
Ft_Gpu_Hal_WrCmd32(phost, 0);
#if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
	SAMAPP_Load_Data_To_CmdBuff("..\\..\\..\\Test\\lenaface40PalettedRGB565.png");
#elif defined(FT900_PLATFORM)
	Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Lenaface40_Paletted_L8_PNG_Hex_Dump, SAMAPP_LENAFACE40_PALETTED_L8_PNG_HEX_DUMP_SIZE);
#endif
//Ft_App_WrCoCmd_Buffer(phost, PALETTE_SOURCE(bitmapHeight*bitmapWidth));
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
//Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(0)); //the indexes start at 1024 byte offset
//Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(PALETTED565, bitmapWidth*2, bitmapHeight)); //The stride for the paletted format PALETTED8 is 1 byte per pixel and height is teh same as the bitmap height
//Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, bitmapWidth, bitmapHeight));
Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center
...
{
	ft_uint32_t dloffset = Ft_Gpu_Hal_Rd32(phost,REG_CMD_DL);
	printf("%d\n",dloffset);
	dloffset += RAM_DL;
for(dlList = RAM_DL;dlList<dloffset;dlList+=4)
{
	//for(dlList = RAM_DL;dlList<(RAM_DL+Ft_Gpu_Hal_Rd32(phost,REG_CMD_DL));dlList+=4)
	printf("0x%08x\n",Ft_Gpu_Hal_Rd32(phost,dlList));
}
...
Ft_Gpu_Hal_WrCmd32(phost, CMD_LOADIMAGE);
Ft_Gpu_Hal_WrCmd32(phost, 10000);	//destination address if inflate
Ft_Gpu_Hal_WrCmd32(phost, 0);
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_LENAFACE40_PALETTED_RGB565_PNG_HEX_DUMP, SAMAPP_LENAFACE40_PALETTED_RGB565_PNG_HEX_DUMP_SIZE);
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(10000)); //the indexes start at 1024 byte offset
Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(PALETTED4444, bitmapStride, bitmapHeight)); //The stride for the paletted format PALETTED8 is 1 byte per pixel and height is teh same as the bitmap height
Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, bitmapWidth, bitmapHeight));
Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center
printf("Constructed RGB565\n");
...
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 95, 31, OPT_CENTER, "Original: 32KB.");
Ft_Gpu_Hal_WrCmd32(phost, CMD_LOADIMAGE);
Ft_Gpu_Hal_WrCmd32(phost, 50000);	//destination address if inflate
Ft_Gpu_Hal_WrCmd32(phost, 0);
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_LENAFACE40_PALETTED_ARGB4_PNG_HEX_DUMP, SAMAPP_LENAFACE40_PALETTED_ARGB4_PNG_HEX_DUMP_SIZE);
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(50000)); //the indexes start at 1024 byte offset
Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(ARGB4, bitmapStride*2, bitmapHeight)); //The stride for the paletted format PALETTED8 is 1 byte per pixel and height is teh same as the bitmap height
Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, bitmapWidth, bitmapHeight));
Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center
printf("Constructed paletted4444\n");


/*
Ft_App_WrCoCmd_Buffer(phost, PALETTE_SOURCE(RAM_G)); //specify the palette table address
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(1024)); //the indexes start at 1024 byte offset
Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(PALETTED8, bitmapStride, bitmapHeight)); //The stride for the paletted format PALETTED8 is 1 byte per pixel and height is teh same as the bitmap height
Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, bitmapWidth, bitmapHeight));
Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth / 2 - bitmapWidth / 2) * 16, (FT_DispHeight / 2 - bitmapHeight / 2) * 16)); //draw the image in the center
Ft_App_WrCoCmd_Buffer(phost, END());
Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(255, 0, 0));

Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 50, 31, OPT_CENTER,
		"Paletted Bitmap: 16KB.");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 95, 31, OPT_CENTER,
		"Original: 32KB.");
		*/
...
ft_uint8_t tagval, RDialTag = 100, GDialTag = 101, BDialTag = 102, ADialTag =
		103, DialR = 80, i, j, rectRed, rectGreen, rectBlue, rectAlpha = 255;
ft_uint16_t RDialX = DialR + 20, RDialY = FT_DispHeight / 4, GDialX = DialR
		...
		* 3, rectWidth = 300, rectHeight = 300, rectX = FT_DispWidth / 2
...
while (delayLoop != 0) {
	...
	Ft_App_WrCoCmd_Buffer(phost,
			VERTEX2F((rectX + rectWidth) * 16, (rectY + rectHeight) * 16));
...
#elif defined(FT900_PLATFORM)
fResult = f_open(&CurFile, "0:\csf_qhd.avi", FA_READ | FA_OPEN_EXISTING);
if (fResult == FR_OK)
{
	printf("\nFopen success!\n");
	fResult = f_lseek(&CurFile,0);
...
/*
Ft_Gpu_CoCmd_LoadIdentity(phost);
Ft_Gpu_CoCmd_Scale(phost, 78643 , 73728); //65536*1.2, 65536*1.125
Ft_Gpu_CoCmd_SetMatrix(phost);
*/
	...
	filesz = f_size(&CurFile);
#endif
	Ft_Gpu_CoCmd_PlayVideo(phost,OPT_FULLSCREEN | OPT_NOTEAR);
	Ft_App_Flush_Co_Buffer(phost);
	//Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
	{
...
if (fResult != FR_OK) {
	...
	/* start video playback, load the data into media fifo */
	mediafifo = 854 * 480 * 2L + 32 * 1024L; //starting address of the media fifo
	...
	Ft_Fifo_Init(&stFifo, mediafifo, mediafifolen, REG_MEDIAFIFO_READ,
	REG_MEDIAFIFO_WRITE); //initialize application media fifo structure
#if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
			fseek(pFile,0,SEEK_END);
			filesz = ftell(pFile);
			fseek(pFile,0,SEEK_SET);
	...
	filesz = f_size(&CurFile);
#endif
	Ft_Gpu_CoCmd_MediaFifo(phost, mediafifo, mediafifolen); //Setup media fifo buffer
	Ft_App_Flush_Co_Buffer(phost);
	Ft_Gpu_Hal_WaitCmdfifo_empty(phost);

	Ft_Gpu_CoCmd_PlayVideo(phost, OPT_MEDIAFIFO | OPT_FULLSCREEN | OPT_NOTEAR); //start video playback and the device will take over after flushing the command, the application just need to feed video data to the device for the video playback.
	...
	filesz -= stFifo.fifo_wp;
	while (cmdrd != cmdwr)  //loop till end of the file
	{
		if (filesz > 0) {
			if (filesz > SCRATCH_BUFF_SZ) {
				currchunk = SCRATCH_BUFF_SZ;
...
//function to demonstrate new screen shot api.
ft_void_t ScreenShot2() {
ft_uint16_t imgWidth = 800, imgHeight = 480;
ft_uint16_t stride = imgWidth * 2;  //RGB565 format is 2 bytes per pixel
ft_uint32_t FileLen, ReadWord, ScreenShotAddr = imgWidth * 2 * imgHeight;
ft_uchar8_t *pbuff;
ft_uint8_t touched = 0, exitLoop = 0, i;
ft_uint16_t startingX = 0, startingY = 0, endX = 0, endY = 0, FinalWidth,
		FinalHeight, FinalX, FinalY, centerX, centerY;
ft_int16_t backgroundAlpha = 255;
static int RectWidth, RectHeight;
#if defined(FT900_PLATFORM)
ft_uint16_t exitLoopCounter = 500;
#else
ft_uint16_t exitLoopCounter=250;
#endif

#if (0)
//load bitmap from data array
//Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G,(ft_uchar8_t *)fbuff,  FileLen);
FILE *pFile = fopen("..\\..\\..\\Test\\EiffelTower_800_480.jpg","rb");
...
Ft_App_WrCoCmd_Buffer(phost, BITMAP_LAYOUT_H((imgWidth*2)>>10,imgHeight>>9)); //the two most significant bits of stride and height are specified in this API.  NOTE: the stride is 10bits while height and width is 9 bits
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE(NEAREST,BORDER,BORDER,imgWidth,imgHeight));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE_H(imgWidth>>9,imgHeight>>9));////the two most significant bits of width and height are specified in this API
#endif
...
Ft_Gpu_CoCmd_Dlstart(phost);
Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2), 30,
OPT_CENTER, "Touch and drag a portion of the next screen");
Ft_App_WrCoCmd_Buffer(phost, DISPLAY());
Ft_Gpu_CoCmd_Swap(phost);
Ft_App_Flush_Co_Buffer(phost);
Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
SAMAPP_ENABLE_DELAY_VALUE(2500);
Ft_Gpu_Hal_Wr8(phost, REG_CTOUCH_EXTENDED, CTOUCH_MODE_EXTENDED);
while (!exitLoop) {
	Ft_Gpu_CoCmd_Dlstart(phost);
	...
	RectWidth = FT_DispWidth;
	RectHeight = FT_DispHeight;
	//draw the background
	for (i = 8; i > 0; i--) {
		if (i == 1)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(25, 196, 103));
		else if (i == 2)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(72, 83, 134));
		else if (i == 3)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(100, 11, 24));
		else if (i == 4)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(51, 116, 248));
		else if (i == 5)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(66, 167, 21));
		else if (i == 6)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(18, 87, 97));
		else if (i == 7)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(103, 243, 56));
		else if (i == 8)
			Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(75, 14, 96));
		Ft_App_WrCoCmd_Buffer(phost, LINE_WIDTH(1 * 16)); //line_width alters the rectangle's curvature
		Ft_App_WrCoCmd_Buffer(phost,
				VERTEX2F(((FT_DispWidth / 2) - (RectWidth / 2)) * 16,
						(FT_DispHeight / 2 - RectHeight / 2) * 16));
		Ft_App_WrCoCmd_Buffer(phost,
				VERTEX2F(((FT_DispWidth / 2) + (RectWidth / 2)) * 16,
						(FT_DispHeight / 2 + RectHeight / 2) * 16));
		RectWidth -= (FT_DispWidth >> 3);
		RectHeight -= (FT_DispHeight >> 3);
	}

	//ReadWord = Ft_Gpu_Hal_Rd32(phost, REG_TOUCH_SCREEN_XY);
	ReadWord = Ft_Gpu_Hal_Rd32(phost, REG_CTOUCH_TOUCH0_XY);
	if (touched == 0 && ReadWord != 0x80008000) {
		touched = 1;
		startingY = (ReadWord & 0xffff);
		startingX = (ReadWord >> 16);
	} else if (touched == 1 && ReadWord != 0x80008000) {
		endY = (ReadWord & 0xffff);
		endX = (ReadWord >> 16);
		Ft_App_WrCoCmd_Buffer(phost, COLOR_RGB(255, 0, 0));
		Ft_App_WrCoCmd_Buffer(phost, LINE_WIDTH(1 * 16));
		Ft_App_WrCoCmd_Buffer(phost, BEGIN(LINES));
		...
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(endX * 16, startingY * 16));

		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(endX * 16, startingY * 16));
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(endX * 16, endY * 16));

		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(startingX * 16, startingY * 16));
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(startingX * 16, endY * 16));

		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(startingX * 16, endY * 16));
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(endX * 16, endY * 16));
	} else if (touched == 1 && ReadWord == 0x80008000
			&& abs(endX - startingX) > 10 && abs(endY - startingY) > 10) {
		FinalWidth = abs(endX - startingX);
		FinalHeight = abs(endY - startingY);
		if (startingX > endX) {
			FinalX = endX;
			FinalY = endY;
		} else {
		...
		centerX = FT_DispWidth / 2 - FinalWidth / 2;
		centerY = FT_DispHeight / 2 - FinalHeight / 2;
		Ft_App_WrCoCmd_Buffer(phost, BITMAP_HANDLE(2));
		Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(ScreenShotAddr));
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_LAYOUT(RGB565, (FinalWidth * 2), FinalHeight));
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_LAYOUT_H((FinalWidth * 2) >> 10, FinalHeight >> 9)); //in case one of the dimension of the selected area is greater than 512
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_SIZE(NEAREST, BORDER, BORDER, FinalWidth, FinalHeight));
		Ft_App_WrCoCmd_Buffer(phost,
				BITMAP_SIZE_H(FinalWidth >> 9, FinalHeight >> 9)); //in case one of the dimension of the selected area is greater than 512
		exitLoop = 1;
	}
...
if ((FinalX != centerX && FinalY != centerY))
	Ft_Gpu_CoCmd_Snapshot2(phost, RGB565, ScreenShotAddr, FinalX, FinalY,
...
/*
 ...
 Ft_App_WrCoCmd_Buffer(phost,CLEAR(1,1,1));
 Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
 Ft_App_WrCoCmd_Buffer(phost,COLOR_A(backgroundAlpha));
 Ft_App_WrCoCmd_Buffer(phost, BITMAP_HANDLE(2));
 Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(0,0));
 Ft_App_WrCoCmd_Buffer(phost,COLOR_A(255));
 Ft_App_WrCoCmd_Buffer(phost, BITMAP_HANDLE(2));
 Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(FinalX*16,FinalY*16));
 Ft_App_WrCoCmd_Buffer(phost,DISPLAY());
 Ft_Gpu_CoCmd_Swap(phost);
 Ft_App_Flush_Co_Buffer(phost);
 Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
...
while (FinalX != centerX && FinalY != centerY) {
	FinalX =
			FinalX > centerX ?
					(FinalX - (1 + (FinalX - centerX) / 32)) :
					(FinalX + (1 + (centerX - FinalX) / 32));
	FinalY =
			FinalY > centerY ?
					(FinalY - (1 + (FinalY - centerY) / 32)) :
					(FinalY + (1 + (centerY - FinalY) / 32));
	Ft_Gpu_CoCmd_Dlstart(phost);
	Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
	Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
	Ft_App_WrCoCmd_Buffer(phost, COLOR_A(255));
...
ft_uint16_t imgWidth = 128, imgHeight = 128, c0_c1_width = 32,
		c0_c1_height = 32, c0_c1_stride = c0_c1_width * 2, b0_b1_width =
				imgWidth, b0_b1_height = imgHeight, b0_b1_stride = b0_b1_width
				/ 8, szPerFile = 2048;

#if defined(ARDUINO_PLATFORM) || defined(FT900_PLATFORM)
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G, SAMAPP_Tomato_DXT1_C0_Data_Raw,
		szPerFile);
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G + szPerFile,
		SAMAPP_Tomato_DXT1_C1_Data_Raw, szPerFile);
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G + szPerFile * 2,
		SAMAPP_Tomato_DXT1_B0_Data_Raw, szPerFile);
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G + szPerFile * 3,
		SAMAPP_Tomato_DXT1_B1_Data_Raw, szPerFile);
#endif
...
SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_128_128_c0.raw", RAM_G);
SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_128_128_c1.raw", RAM_G + szPerFile);
SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_128_128_b0.raw", RAM_G + szPerFile * 2);
SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_128_128_b1.raw", RAM_G + szPerFile * 3);

//SAMAPP_load_inflated_image("..\\..\\..\\Test\\combined.bin", 0);
#endif
...
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2II(FT_DispWidth / 2 - b0_b1_width / 2,
				FT_DispHeight / 2 - b0_b1_height / 2, 1, 0));
Ft_App_WrCoCmd_Buffer(phost, BLEND_FUNC(ONE, ONE));
Ft_App_WrCoCmd_Buffer(phost, COLOR_A(0xAA));
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2II(FT_DispWidth / 2 - b0_b1_width / 2,
				FT_DispHeight / 2 - b0_b1_height / 2, 1, 1));
Ft_App_WrCoCmd_Buffer(phost, COLOR_MASK(1, 1, 1, 0));
...
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2II(FT_DispWidth / 2 - b0_b1_width / 2,
				FT_DispHeight / 2 - b0_b1_height / 2, 2, 1));
Ft_App_WrCoCmd_Buffer(phost, BLEND_FUNC(ONE_MINUS_DST_ALPHA, ONE));
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2II(FT_DispWidth / 2 - b0_b1_width / 2,
				FT_DispHeight / 2 - b0_b1_height / 2, 2, 0));
Ft_App_WrCoCmd_Buffer(phost, END());
...
ft_void_t LargerFontSupport() {
/*
 FT81x can support larger rom fonts than FT80x and the larger rom fonts need to be associated with a handle before they can be used.
 */
Ft_Gpu_CoCmd_Dlstart(phost);
Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
Ft_Gpu_CoCmd_RomFont(phost, 1, 31); //assign rom font handle 31 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2), 1, 0, "31");
Ft_Gpu_CoCmd_RomFont(phost, 1, 32); //assign rom font handle 32 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2) + 60, 1, 0,
		"32");
Ft_Gpu_CoCmd_RomFont(phost, 1, 33); //assign rom font handle 33 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2) + 80, (FT_DispHeight / 2) - 14, 1,
		0, "33");
Ft_Gpu_CoCmd_RomFont(phost, 1, 34); //assign rom font handle 34 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2) + 60, (FT_DispHeight / 2) + 32, 1,
		0, "34");

Ft_Gpu_CoCmd_RomFont(phost, 1, 31); //assign rom font handle 31 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (200 / 2), (200 / 2), 1, 0, "31");
Ft_Gpu_CoCmd_RomFont(phost, 1, 32); //assign rom font handle 32 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (200 / 2), (200 / 2) + 60, 1, 0, "32");
Ft_Gpu_CoCmd_RomFont(phost, 1, 33); //assign rom font handle 33 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (200 / 2) + 80, (200 / 2) - 14, 1, 0, "33");
Ft_Gpu_CoCmd_RomFont(phost, 1, 34); //assign rom font handle 34 to bitmap handle 1
Ft_Gpu_CoCmd_Text(phost, (200 / 2) + 60, (200 / 2) + 32, 1, 0, "34");
...
#if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tuffy.ttf_24_L1.raw", fontFileAddress); //load font data consits of font metric block and font data from the FTDI font conversion utility.  The file has been packaged with the font metric block at the beginning of the font data.
#elif defined(ARDUINO_PLATFORM) || defined(FT900_PLATFORM)
Ft_Gpu_Hal_WrMemFromFlash(phost, RAM_G, SAMAPP_Lowercase_Alphabet_Data, 1136);
#endif
...
//l1
Ft_App_WrCoCmd_Buffer(phost, BITMAP_LAYOUT(L1, 3, 29)); //font stride and height metrics can be found in the .rawh file
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE(NEAREST, BORDER, BORDER, 20, 29)); //font width and height metrics can be found in the .rawh file

Ft_Gpu_CoCmd_SetFont2(phost, 1, fontFileAddress, 32); //set ram font to font handle 1, font metric block starts at fontFileAddress(RAM_G), and set the starting character to the 32th character in the set.
//Ft_Gpu_CoCmd_SetFont2(phost, 1, RAM_G, 97);

/*
 The lower case alphabets don't have the space character
 */
//Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2) - 80, 1, OPT_CENTER, "The quick brown fox jumps");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2) - 80, 1, OPT_CENTER, "e");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2), 1, OPT_CENTER, "over the lazy dog.");
Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), (FT_DispHeight / 2) + 80, 1, OPT_CENTER, "abcdefghijklmnopqrstuvwxyz");

Ft_App_WrCoCmd_Buffer(phost, DISPLAY());
Ft_Gpu_CoCmd_Swap(phost);
Ft_App_Flush_Co_Buffer(phost);
Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
SAMAPP_ENABLE_DELAY_VALUE(50000);
}

/*
 ft_void_t test_images(){

 Ft_Gpu_CoCmd_Dlstart(phost);
 Ft_App_WrCoCmd_Buffer(phost,CLEAR(1,1,1) );
 Ft_App_WrCoCmd_Buffer(phost,COLOR_RGB(255,255,255));
 SAMAPP_load_inflated_image("..\\..\\..\\Test\\shirt.bin", 0);  //load font data consits of font metric block and font data from the FTDI font conversion utility.  The file has been packaged with the font metric block at the beginning of the font data.

 Ft_App_WrCoCmd_Buffer(phost,BITMAP_HANDLE(1)); //associate to font handle 1
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(0)); //The starting address for the font file 148 bytes into the font file because the first 148 bytes are font metric informational block.
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(ARGB4,80,40));  //font stride and height metrics can be found in the .rawh file
 Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST,BORDER,BORDER,40,40)); //font width and height metrics can be found in the .rawh file
...
Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
#if defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU)
SAMAPP_LoadRawFromFile("..\\..\\..\\Test\\Tomato_ARGB4_128_128.raw", RAM_G);
#elif defined(FT900_PLATFORM) || defined(ARDUINO_PLATFORM)
Ft_Gpu_Hal_WrCmd32(phost, CMD_INFLATE);
Ft_Gpu_Hal_WrCmd32(phost, RAM_G); //destination address if inflate
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_Bitmap_Data_Bin, 4509);
#endif

Ft_App_WrCoCmd_Buffer(phost, BITMAP_HANDLE(15));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SOURCE(RAM_G));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_LAYOUT(ARGB4, 256, 128));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE(NEAREST, BORDER, BORDER, 128, 128));
Ft_App_WrCoCmd_Buffer(phost, DISPLAY());
Ft_Gpu_CoCmd_Swap(phost);
Ft_App_Flush_Co_Buffer(phost);
Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
...
//normally the built-in buttons, keys and graident widget uses bitmap handle 15, setscratch command changes the scratch bitmap handle.
Ft_Gpu_CoCmd_SetScratch(phost, 0);

Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 50, 31, OPT_CENTER,
		"Landscape Mode");  //The default is landscape mode
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2II(FT_DispWidth / 2 - 64, FT_DispHeight / 2 - 64, 15, 0));
Ft_App_WrCoCmd_Buffer(phost, DISPLAY());
Ft_Gpu_CoCmd_Swap(phost);
Ft_App_Flush_Co_Buffer(phost);
Ft_Gpu_Hal_WaitCmdfifo_empty(phost);
...
Ft_Gpu_CoCmd_Dlstart(phost);
Ft_App_WrCoCmd_Buffer(phost, CLEAR(1, 1, 1));
Ft_Gpu_CoCmd_SetRotate(phost, 90);
Ft_Gpu_CoCmd_Text(phost, (FT_DispHeight / 2), 50, 31, OPT_CENTER,
		"Portrait Mode"); //In portrait mode (90 degree counter clockwise rotation), the x and y components are flipped.
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
Ft_App_WrCoCmd_Buffer(phost,
		VERTEX2II(FT_DispHeight / 2 - 64, FT_DispWidth / 2 - 64, 15, 0));
...
ft_uint16_t stride = imgWidth * 2;  //RGB565 format is 2 bytes per pixel
	...
	Ft_Gpu_Hal_WrCmd32(phost, RAM_G);  //destination address if inflate
	Ft_Gpu_Hal_WrCmd32(phost, 0);
	SAMAPP_Load_Data_To_CmdBuff("..\\..\\..\\Test\\EiffelTower_800_480.jpg");
#elif defined(FT900_PLATFORM) || defined(ARDUINO_PLATFORM)
Ft_Gpu_Hal_WrCmd32(phost, CMD_INFLATE);
Ft_Gpu_Hal_WrCmd32(phost, RAM_G);  //destination address if inflate
Ft_Gpu_Hal_WrCmdBufFromFlash(phost, SAMAPP_Tomato_Bitmap_Data_Bin, 4509);
...
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE(NEAREST, BORDER, BORDER, imgWidth, imgHeight));
Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE_H(imgWidth>>9, imgHeight>>9));
Ft_App_WrCoCmd_Buffer(phost, BEGIN(BITMAPS));
...
//SAMAPP_ENABLE_DELAY_VALUE(1000);
//SAMAPP_API_Screen("Audio is now disabled.");
//SAMAPP_CoPro_Widget_Calibrate();
...
//Ft_GPU_81X_PadDriveStrength(phost, FT_GPU_10MA, FT_GPU_GPIO1);  //change GPIO1
//Ft_GPU_81X_PadDriveStrength(phost, FT_GPU_10MA, FT_GPU_GPIO2);  //change GPIO2
//Ft_GPU_81X_PadDriveStrength(phost, FT_GPU_10MA, FT_GPU_GPIO3);  //change GPIO3
//Ft_GPU_81X_PadDriveStrength(phost, FT_GPU_10MA, FT_GPU_GPIO4);  //change GPIO4
...
ft_void_t testSetrotate(){
	ft_uint8_t setrotateoption=0;

	//Ft_Gpu_Hal_Wr32(phost, REG_ROTATE,setrotateoption);
	while(1){
			Ft_Gpu_CoCmd_Dlstart(phost);
			Ft_App_WrCoCmd_Buffer(phost,CLEAR(1,1,1));
			Ft_App_WrCoCmd_Buffer(phost,COLOR_RGB(255,255,255));
			Ft_Gpu_CoCmd_Text(phost, (FT_DispWidth / 2), 50, 31, OPT_CENTER, "Touch the white dot");  //The default is landscape mode
			Ft_Gpu_CoCmd_Dial(phost, FT_DispWidth-50, 50, 30, 0, 15626);
			Ft_Gpu_CoCmd_Slider(phost, 500, 100, 50,10, 0, 35535, 65535);
			Ft_Gpu_CoCmd_Number(phost, (FT_DispWidth / 2), 100, 31, 0, setrotateoption);
			Ft_App_WrCoCmd_Buffer(phost,BEGIN(LINES));
			Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((0)*16,(0)*16));
			Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((FT_DispWidth)*8,(FT_DispHeight)*8));
			Ft_App_WrCoCmd_Buffer(phost,BEGIN(FTPOINTS));
			Ft_App_WrCoCmd_Buffer(phost,POINT_SIZE(20*16));
			...
			if(Read_Keys()=='P'){
				setrotateoption++;
				printf("Rotation option: %d\n",setrotateoption);
				if(setrotateoption > 7){
					setrotateoption=0;
					printf("New Rotation option: %d\n",setrotateoption);
				}
				//Ft_Gpu_Hal_Wr32(phost, REG_ROTATE,setrotateoption);
				//Ft_Gpu_Hal_Sleep(2000);
				Ft_Gpu_CoCmd_SetRotate(phost, setrotateoption);
			}
	}
...
ft_void_t fontTesting() {

ft_uint32_t ptr, Wptr;
ft_uint8_t Width = 0, i = 16, j, k, character;
...
for (i = 16; i <= 34; i++) {
	...
	printf("NULL :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 0));
	printf("SOH  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 1));
	printf("STX  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 2));
	printf("ETX  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 3));
	printf("EOT  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 4));
	printf("ENQ  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 5));
	printf("ACK  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 6));
	printf("BEL  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 7));
	printf("BS   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 8));
	printf("HT   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 9));
	printf("LF   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 10));
	printf("VT   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 11));
	printf("FF   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 12));
	printf("CR   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 13));
	printf("SO   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 14));
	printf("SI   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 15));
	printf("DLE  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 16));
	printf("DC1  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 17));
	printf("DC2  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 18));
	printf("DC3  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 19));
	printf("DC4  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 20));
	printf("NAK  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 21));
	printf("SYN  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 22));
	printf("ETB  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 23));
	printf("CAN  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 24));
	printf("EM   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 25));
	printf("SUB  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 26));
	printf("ESC  :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 27));
	printf("FS   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 28));
	printf("GS   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 29));
	printf("RS   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 30));
	printf("US   :  %d\n", Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + 31));

	for (k = 32; k <= 127; k++) {
		printf("%c  :  %d\n", k,
				Ft_Gpu_Hal_Rd8(phost, ptr + (i - 16) * 148 + k));
	}

	printf("Font bitmap format(L1,L4,L8): %d\n",
			Ft_Gpu_Hal_Rd32(phost, ptr + (i - 16) * 148 + 128));
	printf("Font line stride, in bytes: %d\n",
			Ft_Gpu_Hal_Rd32(phost, ptr + (i - 16) * 148 + 132));
	printf("Font width, in pixels: %d\n",
			Ft_Gpu_Hal_Rd32(phost, ptr + (i - 16) * 148 + 136));
	printf("Font height, in pixels: %d\n",
			Ft_Gpu_Hal_Rd32(phost, ptr + (i - 16) * 148 + 140));
	printf("Pointer to font graphics data in memory: %d\n",
			Ft_Gpu_Hal_Rd32(phost, ptr + (i - 16) * 148 + 144));
}
...
//	SAMAPP_API_Screen("Larger built-in font support.");
//	LargerFontSupport();
	...
	//SAMAPP_CoPro_Widget_Calibrate();
	//SAMAPP_API_Screen("Updated Screen Capture API");
	//ScreenShot2();
#if defined(FT900_PLATFORM)
	if (SDHostStatus == SDHOST_CARD_INSERTED) 
#endif
	...
	//SAMAPP_API_Screen("Video frame playback");
	//VideoPlayBackFrameByFrame();  //little more work
