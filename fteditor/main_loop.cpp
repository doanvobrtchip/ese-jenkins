...
// Array indexed by display list index containing coprocessor line which wrote
...
static int *s_DisplayListCoprocessorCommandWrite =
    s_DisplayListCoprocessorCommandB;
...
static uint32_t (*s_CoCmdReadValuesWrite)[DL_PARSER_MAX_READOUT] =
    s_CoCmdReadValuesB;
...
void setup() {
  ...
  wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_INT_MASK),
       0xFF);  // INT_CMDEMPTY);
  wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_INT_EN), 1);

  if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
    // Calling this on first emulator launch fixes an odd coprocessor issue with
    // fonts not getting loaded occasionally
...
void resetCoprocessorFromLoop() {
  ...
  bool needCoproPatch = FTEDITOR_CURRENT_DEVICE == FTEDITOR_BT815 ||
  ...
  QThread::msleep(10);  // Timing hack because we don't lock CPURESET flag at
                        // the moment with coproc thread
  // Leave reset
  if (hasOTP()) {
  ...
  QThread::msleep(100);  // Timing hack because we don't lock CPURESET flag at
                         // the moment with coproc thread
  if (hasOTP()) {
    ...
    QThread::msleep(10);  // Timing hack because it's not checked when the
                          // coprocessor finished processing the CMD_EXECUTE
  ...
  {
          // Attach flash
          wr32(addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_CMD) + 4,
  CMD_FLASHATTACH); wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_WRITE),
  8);
  }
  else
  */
...
void loop() {
  ...
  if (BT8XXEMU_hasInterrupt(g_Emulator)) {
    ...
    // printf("INTERRUPT %i\n", flags);
    if ((rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_READ)) & 0xFFF) ==
        0xFFF) {
  ...
  bool contentPoked =
      false;  // Whether the contents of RAM_G or Flash has been changed
  ...
  if (coprocessorSwapped) {
    ...
    while (rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_DLSWAP)) !=
           DLSWAP_DONE) {
      if (!g_EmulatorRunning) return;
    }
    displayListSwapped = false;
  ...
  if (g_Flash) {
    ...
    for (std::set<ContentInfo *>::iterator it(contentInfoFlash.begin()),
         end(contentInfoFlash.end());
         it != end; ++it) {
      ContentInfo *info = (*it);
      int loadAddr =
          info->FlashAddress;  // (info->Converter == ContentInfo::Image) ?
                               // info->bitmapAddress() : info->MemoryAddress;
      if (loadAddr < FTEDITOR_FLASH_FIRMWARE_SIZE) {
        // Safety to avoid breaking functionality, never allow overriding the
        // provided firmware from the content manager
        printf("[Flash] Error: Load address not permitted for '%s' to '%i'\n",
               info->DestName.toLocal8Bit().data(), loadAddr);
        continue;
      }
      bool dataCompressed = (info->Converter != ContentInfo::ImageCoprocessor &&
                             info->Converter != ContentInfo::FlashMap)
                                ? info->DataCompressed
                                : false;
      ...
      };
      {
        ...
        char *ram = static_cast<char *>(
            static_cast<void *>(BT8XXEMU_Flash_data(g_Flash)));
  ...
  for (std::set<ContentInfo *>::iterator it(contentInfoMemory.begin()),
       end(contentInfoMemory.end());
       it != end; ++it) {
    ContentInfo *info = (*it);
    int loadAddr = (info->Converter == ContentInfo::Image)
                       ? info->bitmapAddress()
                       : info->MemoryAddress;
    ...
    if (imageCoprocessor)  // Load image through coprocessor
    {
      ...
      for (;;) {
        if (freespace < (4 + 8) || writeCount < 128) {
          swrend();
          wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_WRITE),
               (wp & 0xFFF));
          do {
            if (!g_EmulatorRunning) {
          ...
          swrbegin(addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_CMD) +
                   (wp & 0xFFF));
        }
        uint32_t buffer;
    ...
    // ok
    {
      ...
      int s = in.readRawData(
          &ram[FTEDITOR::addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_G) +
               loadAddr],
          binSize);
	  binFile.close();
      BT8XXEMU_poke(g_Emulator);
    ...
    if (FTEDITOR_CURRENT_DEVICE < FTEDITOR_FT810) {
      if (info->Converter == ContentInfo::Image &&
          info->ImageFormat == PALETTED) {
        ...
        // ok
        {
          ...
          int s = in.readRawData(
              &ram[addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_PAL)], palSize);
          BT8XXEMU_poke(g_Emulator);
          contentPoked = true;
        }
      }
    ...
    if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810) {
      if (info->Converter == ContentInfo::Image &&
          (info->ImageFormat == PALETTED8 || info->ImageFormat == PALETTED565 ||
           info->ImageFormat == PALETTED4444)) {
        int palSize;
        ...
        //{
        //	printf("[RAM_PAL] Error: File of size '%i' not equal to
        //palSize\n", palSize); 	continue;
        // }
        //  ok
        {
  ...
  /*bool reuploadBitmapSetup = contentInfo.size() || s_BitmapSetupModNb <
  s_BitmapSetup->getModificationNb(); if (reuploadBitmapSetup)
  {
          printf("Reupload bitmap setup to RAM_DL\n");
          wr32(REG_PCLK, 0);
          swrbegin(RAM_DL);
          const FT800EMU::BitmapInfo *bitmapInfo =
  s_BitmapSetup->getBitmapInfos(); const ContentInfo *const *bitmapSources =
  s_BitmapSetup->getBitmapSources(); for (int i = 0; i <
  BITMAP_SETUP_HANDLES_NB; ++i)
          {
                  ...
                  if (info && s_BitmapSetup->bitmapSourceExists(i))
                  {
                          ...
                          // printf("%i, %i, %i\n", info->ImageFormat,
  info->CachedImageStride, info->CachedImageHeight); if (info->Converter ==
  ContentInfo::Image) // Always use cached data from content info for image
  layout swr32(BITMAP_LAYOUT(info->ImageFormat, info->CachedImageStride,
  info->CachedImageHeight)); else
                                  swr32(BITMAP_LAYOUT(bitmapInfo[i].LayoutFormat,
  bitmapInfo[i].LayoutStride, bitmapInfo[i].LayoutHeight));
                          swr32(BITMAP_SIZE(bitmapInfo[i].SizeFilter,
  bitmapInfo[i].SizeWrapX, bitmapInfo[i].SizeWrapY, bitmapInfo[i].SizeWidth,
  bitmapInfo[i].SizeHeight));
                  }
          }
          swrend();
  ...
  if (contentInfoMemory.size()) g_CoprocessorContentSuccess = true;
  g_ContentManager->unlockContent();
  ...
  // if (macroModified) // Always write macros to intial user value, in case
  // changed by coprocessor
  // {
  if (g_Macro->getDisplayListParsed()[0].ValidId &&
      rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_MACRO_0)) !=
          g_Macro->getDisplayList()[0])  // Do a read test so we don't change
                                         // the ram if not necessary
    wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_MACRO_0),
         g_Macro->getDisplayList()[0]);  // (because ram writes cause the write
                                         // count to increase and force a
                                         // display render)
  if (g_Macro->getDisplayListParsed()[1].ValidId &&
      rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_MACRO_1)) !=
          g_Macro->getDisplayList()[1])
    wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_MACRO_1),
         g_Macro->getDisplayList()[1]);
  // }
  ...
  if (dlModified || cmdModified ||
      /*reuploadFontSetup ||*/ (g_StepCmdLimit != s_StepCmdLimitCurrent) ||
      s_WantReloopCmd || (s_HasContentReadCoCmd && contentPoked)) {
    ...
    for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
      // printf("dl %i: %i\n", i, displayList[i]);
      if ((displayList[i] & ~(CLEAR(0, 0, 0) ^ CLEAR(1, 1, 1))) ==
          CLEAR(0, 0, 0))
    ...
    for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
      ...
      DlParser::compile(FTEDITOR_CURRENT_DEVICE, s_CmdParamCache,
                        cmdParsedPtr[i]);
    ...
    bool validCmd = false;
    int coprocessorWrites[1024];  // array indexed by write pointer of command
                                  // index in the coprocessor editor gui
    for (int i = 0; i < 1024; ++i) coprocessorWrites[i] = -1;
    for (int i = 0; i < FTEDITOR_DL_SIZE; ++i)
      s_DisplayListCoprocessorCommandWrite[i] = -1;
    for (int i = 0; i < FTEDITOR_DL_SIZE; ++i)
      s_CoCmdReadIndicesWrite[i] = -1;  // wipe
    int coCmdReadNb = 0;
    uint32_t *ramCmd = reinterpret_cast<uint32_t *>(&BT8XXEMU_getRam(
        g_Emulator)[addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_CMD)]);
    int wp = rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_WRITE));
    int rp = rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_READ));
    int fullness = ((wp & 0xFFF) - rp) & 0xFFF;
    ...
    freespace -= 4;
    if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
      // CMD_COLDSTART may cause inconsistent co-processor flash state, needs to
      // be checked
      /*
    ...
    for (int i = 0;
         i < (s_StepCmdLimitCurrent ? s_StepCmdLimitCurrent : FTEDITOR_DL_SIZE);
         ++i)  // FIXME CMD SIZE
    {
      ...
      switch (cmdList[i]) {
          ...
          // (That is, the coprocessor itself is reading from RAM_G or Flash,
          // not just the display engine.)
      ...
      if ((FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810) &&
        ...
        useFileStream =
            useFlash ? NULL : s_CmdStrParamCache[strParamRead].c_str();

        if (useFileStream && cmdList[i] == CMD_INFLATE2) {
          QString tmp =
              QString::fromStdString(s_CmdStrParamCache[strParamRead]);
          if (tmp.endsWith(".bin")) {
      ...
      if (useFileStream) {
        if (!QFileInfo::exists(useFileStream)) continue;
      }
      // Warn the user to insert CMD_DLSTART when other commands follow
      // CMD_TESTCARD
      ...
      if (freespace < (cmdLen + 8))  // Wait for coprocessor ready, + 4 for swap
                                     // and display afterwards
      {
        swrend();
        wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_WRITE),
             (wp & 0xFFF));
        ...
        for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
          if (cpWrite[i] >= 0) {
            ...
            // coprocessorWrites[cpWrite[i]] is the index in the cocmd text
            // editor coprocessorWrites maps from ram_cmd to cocmd text editor
            // cpWrite[i] is the cocmd fifo address for a particular display
            // list index
          }
        }
        ...
        for (int i = coprocessorWriteStart; i < coprocessorWriteEnd;) {
          ...
          if (paramNb)  // for now, just read out all cmd with params
          {
            ...
            for (int j = 0; j < std::min(paramNb, DL_PARSER_MAX_READOUT); ++j)
              s_CoCmdReadValuesWrite[coCmdReadNb][j] =
                  ramCmd[i + 1 + j];  // read from cmd ram directly
            ++coCmdReadNb;
          }
      ...
      if (cmdList[i] == CMD_TESTCARD) {
        ...
        // Might want to warn the user if this is not the last command, and
        // cmdList[i + 1] isn't CMD_DLSTART...
        swr32(CMD_DLSTART);
        ...
        int coprocessorWriteEnd =
            (wp & 0xFFF) >>
            2;  // (differs from regular impl, need to read before wait)
        do {
          rp = rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_READ));
        ...
        coprocessorWriteStart =
            0;  // (differs from regular impl, logo resets wp to 0)
        // <- read from coprocessor fifo ram
        ...
        wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_WRITE),
             (wp & 0xFFF));
        while (rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_READ)) !=
               (wp & 0xFFF)) {
          if (!g_EmulatorRunning) {
            g_WaitingCoprocessorAnimation = false;
            return;
          }
          if ((rd32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_CMD_READ)) &
               0xFFF) == 0xFFF) {
            g_WaitingCoprocessorAnimation = false;
            return;
          }
      ...
      if (useFileStream) {
        ...
        if (true)  // Flush first
        {
          ...
          do {
            ...
            if (g_CmdEditor->isDisplayListModified())  // Trap to avoid infinite
                                                       // flush on errors
            {
        ...
        if (useMediaFifo) {
          if (s_MediaFifoSize) {
            ...
            ramaddr mffree;
#define mfwprd()                                                 \
  mfwp = (rd32(reg(FTEDITOR_CURRENT_DEVICE,                      \
                   FTEDITOR_REG_MEDIAFIFO_WRITE)) /*- mfptr*/) % \
         mfsz;
#define mfwpinc(v) \
  {                \
    mfwp += v;     \
...
#define mfwpwr()                                                   \
  wr32(reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_MEDIAFIFO_WRITE), \
       mfwp /*+ mfptr*/);
#define mffreespace \
  ((mfrp > mfwp) ? (mfrp - mfwp - 4) : (mfrp + s_MediaFifoSize - mfwp - 4))
#define mfrprd()                                                  \
  {                                                               \
    mfrp = (rd32(reg(FTEDITOR_CURRENT_DEVICE,                     \
                     FTEDITOR_REG_MEDIAFIFO_READ)) /*- mfptr*/) % \
           mfsz;                                                  \
    mffree = mffreespace;                                         \
  }
            mfwprd();
            ...
            for (;;) {
              if (mffree < 4) {
                ...
                do {
                  ...
                  if (g_CmdEditor
                          ->isDisplayListModified())  // Trap to avoid infinite
                                                      // flush on errors
                  {
              ...
              uint32_t buffer;
              int nb =
                  mfstream.readRawData(reinterpret_cast<char *>(&buffer), 4);
              if (nb > 0) {
                // write
                swr32(buffer);
                writeCount += nb;
...
#undef mffreespace
          ...
          for (;;) {
            if (freespace < (4 + 8) ||
                writeCount < 128)  // Wait for coprocessor free space, + 4 for
                                   // swap and display afterwards
            {
              ...
              int *cpWrite =
                  BT8XXEMU_getDisplayListCoprocessorWrites(g_Emulator);
              for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
                if (cpWrite[i] >= 0) {
                  // printf("A %i\n", i);
                  s_DisplayListCoprocessorCommandWrite[i] =
            ...
            uint32_t buffer;
            int nb =
                cmdStream.readRawData(reinterpret_cast<char *>(&buffer), 4);
            if (nb > 0) {
              // write
    ...
    if (validCmd) {
      ...
      while ((wp & 0xFFF) != rpl) {
        ...
        if (s_AbortTimer.elapsed() > 1000 && !timeoutFlagged) {
          QMetaObject::invokeMethod(g_CmdEditor->mainWindow(),
                                    &MainWindow::popupTimeout,
                                    Qt::QueuedConnection);
          timeoutFlagged = true;
        }

        if (g_CmdEditor->isDisplayListModified() ||
            s_WantReloopCmd)  // Trap to avoid infinite flush on errors
        {
          s_WantReloopCmd = true;
          if (s_AbortTimer.elapsed() > 1000) {
            printf("Abort coprocessor flush (1100)\n");
            QMetaObject::invokeMethod(g_CmdEditor->mainWindow(),
                                      &MainWindow::actResetEmulator,
                                      Qt::QueuedConnection);
            return;
          }
        }
      ...
      s_DisplayListCoprocessorCommandWrite[i], res.c_str());
              }
      }*/
    ...
    std::swap(g_DisplayListCoprocessorCommandRead,
              s_DisplayListCoprocessorCommandWrite);
  ...
  if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810 && s_MediaFifoStream &&
      s_MediaFifoSize) {
...
void setUtilizationDisplayListCmd(int value) {
  if (g_UtilizationDisplayListCmd == value) return;
