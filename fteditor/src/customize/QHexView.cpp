...
void QHexView::updatePositions()
{
	...
	int serviceSymbolsWidth = (ADR_LENGTH + 1) * m_charWidth + GAP_ADR_HEX + GAP_HEX_ASCII;

	int bpLine = (viewport()->width() - serviceSymbolsWidth) / (4 * m_charWidth + GAP_HEX_X + 1); // 3 bytes for hex + 1 bytes for char
	setBytesPerLine(bpLine <= 0 ? 1 : bpLine);

	m_posAddr = 0;
	m_posHex = (ADR_LENGTH + 1) * m_charWidth + GAP_ADR_HEX;
	m_posAscii = m_posHex + m_bytesPerLine * (GAP_HEX_X + 3 * m_charWidth) + GAP_HEX_ASCII;
...
void QHexView::paintEvent(QPaintEvent *event)
{
	...
	if (hasFocus())
	{
		...
		yFocus -= firstLineIdx;
		int cursorX = ((xFocus / 2) * 3) * m_charWidth + (xFocus / 2) * GAP_HEX_X + m_posHex;
	     ...
	     lineIdx += 1, y += lineHeight())
	{
		...
		for (int x = m_posHex, i = 0, xAscii = m_posAscii;
		     i < m_bytesPerLine && ((lineIdx - firstLineIdx) * m_bytesPerLine + i) < data.size();
		     i++, x += wdRect + GAP_HEX_X, xAscii += m_charWidth + 1)
		{
			...
			if (m_UseContentArea)
			{
				     ...
				     --listIdx)
				{
					...
					if (area->contentInfo->MemoryLoaded && globalIndex >= area->start() && globalIndex <= area->end())
					{
			...
			// Draw hex value
			QString val = QString::number((data.at(index) & 0xF0) >> 4, 16) + QString::number((data.at(index) & 0xF), 16);
...
void QHexView::keyPressEvent(QKeyEvent *event)
{
	...
	{
		setCursorPos(2 * m_bytesPerLine * (m_cursorPos / (m_bytesPerLine * 2) + 1) - 2);
		resetSelection(m_cursorPos);
		isVisible = true;
	}
	...
	{
		int pos = m_cursorPos - (m_cursorPos % (2 * m_bytesPerLine)) + (2 * m_bytesPerLine) - 2;
...
void QHexView::onCopy()
{
	if (m_pdata)
	{
		...
		if (res.size() > 0 && (res.at(res.size() - 1) == ' ' || res.at(res.size() - 1) == '\n'))
		{
			res.remove(res.size() - 1, 1);
		}
...
int QHexView::cursorPos(const QPointF &posF)
{
	...
	if (((int)position.x() >= m_posHex) && ((int)position.x() < (m_posHex + (m_bytesPerLine * (3 * m_charWidth + GAP_HEX_X)))))
	{
...
ContentInfo *QHexView::detectCurrentContentInfo()
{
	...
	for (int i = m_contentAreaList.count() - 1; i >= 0; --i)
	{
		...
		if ((beginIdx >= area->start() && beginIdx <= area->end()) && (endIdx >= area->start() && endIdx <= area->end()))
		{
...
bool QHexView::isValidMouseEvent(QMouseEvent *event)
{
	...
	int y = (event->position().toPoint().y() / m_charHeight - 1) * 2 * m_bytesPerLine;
