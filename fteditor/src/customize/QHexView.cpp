...
bool QHexView::showFromAddress(int address) {
  QMutexLocker lock(&m_dataMtx);

  address -= m_startAddress;
  if (m_pdata && address < m_pdata->size() && address >= 0) {
    updatePositions();
...
int QHexView::cursorPos(const QPointF &posF) {
  auto pos = posF.toPoint();
  int x = 0;
  if (m_selectType == Ascii) {
    if (pos.x() < m_posAscii) {
      x = 0;
    } else if (pos.x() > (m_posAscii + m_bytesPerLine * (m_charWidth + 1))) {
      x = m_bytesPerLine - 1;
    } else
      x = (pos.x() - m_posAscii) / (m_charWidth + 1);
  } else if (m_selectType == Hex) {
    if (pos.x() < m_posHex) {
      x = 0;
    } else if (pos.x() >
      ...
      x = (pos.x() - m_posHex) / (3 * m_charWidth + GAP_HEX_X);
  } else
    return -1;
  x = x * 2;
  int firstLineIdx = verticalScrollBar()->value();
  int y = (pos.y() / lineHeight() - 1) * 2 * m_bytesPerLine;
  int calPos = x + y + firstLineIdx * m_bytesPerLine * 2;
  return calPos > 0 ? calPos : x;
}
...
void QHexView::addContentArea(ContentInfo *contentInfo) {
  ...
  contentArea->color = m_colours[m_contentAreaList.size() % m_colours.size()];
...
void QHexView::setSelectedAddress(int address, int length) {
  address -= m_startAddress;
  m_selectInit = m_selectBegin = address * 2;
  m_selectEnd = m_selectBegin + length * 2;
  viewport()->update();
}

void QHexView::setSelectedOffset(int offset, int length) {
...
void QHexView::updateUint() {
  ...
  if (ok) {
    resultUint = qFromBigEndian(resultUint);
    resultStr = QString("0x%1").arg(resultUint, 8, 16, QChar('0'));
    emit uintChanged(resultStr, resultUint);
  }
}
...
bool QHexView::isValidPress(QMouseEvent *event) {
  ...
  if (x >= m_posHex && x <= m_posAscii - GAP_HEX_ASCII) {
    m_selectType = Hex;
    return true;
  } else if (x >= m_posAscii &&
             x <= (m_posAscii + m_bytesPerLine * (m_charWidth + 1))) {
    m_selectType = Ascii;
    return true;
  }
  m_selectType = None;
