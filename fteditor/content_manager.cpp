...
bool ContentInfo::equalsMeta(const ContentInfo *other) const {
  ...
  switch (Converter) {
    case Raw:
      if (RawStart != other->RawStart) return false;
      if (RawLength != other->RawLength) return false;
      break;
    case Image:
      if (ImageFormat != other->ImageFormat) return false;
      break;
    case ImageCoprocessor:
      if (ImageMono != other->ImageMono) return false;
      break;
    case Font:
      if (ImageFormat != other->ImageFormat) return false;
      if (FontSize != other->FontSize) return false;
      if (FontCharSet != other->FontCharSet) return false;
      if (FontOffset != other->FontOffset) return false;
      break;
    case FlashMap:
      if (MappedName != other->MappedName) return false;
      break;
  }
  return true;
}
...
QLabel *addLabeledWidget(QWidget *parent, QVBoxLayout *layout,
...
void addLabeledWidget(QWidget *parent, QVBoxLayout *layout,
                      const QString &label, QWidget *widget0,
                      QWidget *widget1) {
  QHBoxLayout *hbox = new QHBoxLayout();
  QLabel *l = new QLabel(parent);
  l->setText(label);
...
ContentManager::ContentManager(MainWindow *parent)
  ...
  connect(m_ContentList,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
          this, SLOT(selectionChanged(QTreeWidgetItem *, QTreeWidgetItem *)));

  connect(m_ContentList, &ContentTreeWidget::contentDropped, this,
          [=](QString url) { addInternal(QStringList() << url); });
  ...
  rebuildButton->setText(
      /*QString::fromUcs4(refreshIcon) + " " +*/ tr("Rebuild All"));
  ...
  connect(m_MainWindow->propertiesEditor(), SIGNAL(setterChanged(QWidget *)),
          this, SLOT(propertiesSetterChanged(QWidget *)));
  ...
  addLabeledWidget(this, propCommonLayout, tr("Source file: "),
                   m_PropertiesCommonSourceFile, browseSourceFile);
  connect(m_PropertiesCommonSourceFile, SIGNAL(editingFinished()), this,
          SLOT(propertiesCommonSourcePathChanged()));
  connect(browseSourceFile, SIGNAL(clicked()), this,
          SLOT(propertiesCommonSourcePathBrowse()));
  m_PropertiesCommonName = new UndoStackDisabler<QLineEdit>(this);
  m_PropertiesCommonName->setUndoStack(m_MainWindow->undoStack());
  addLabeledWidget(this, propCommonLayout, tr("Name: "),
                   m_PropertiesCommonName);
  connect(m_PropertiesCommonName, SIGNAL(editingFinished()), this,
          SLOT(propertiesCommonDestNameChanged()));
  ...
  m_PropertiesCommonConverterLabel = addLabeledWidget(
      this, propCommonLayout, tr("Converter: "), propCommonConverter);
  connect(m_PropertiesCommonConverter, SIGNAL(currentIndexChanged(int)), this,
          SLOT(propertiesCommonConverterChanged(int)));
  ...
  addLabeledWidget(this, imagePropsLayout, tr("Format: "),
                   m_PropertiesImageFormat);
  connect(m_PropertiesImageFormat, SIGNAL(currentIndexChanged(int)), this,
          SLOT(propertiesImageFormatChanged(int)));
  ...
  addLabeledWidget(this, imageCoprocessorPropsLayout, tr("Mono (Jpeg only): "),
                   m_PropertiesImageMono);
  connect(m_PropertiesImageMono, SIGNAL(stateChanged(int)), this,
          SLOT(propertiesImageMonoChanged(int)));
  ...
  addLabeledWidget(this, fontPropsLayout, tr("Format: "),
                   m_PropertiesFontFormat);
  connect(m_PropertiesFontFormat, SIGNAL(currentIndexChanged(int)), this,
          SLOT(propertiesFontFormatChanged(int)));
  ...
  connect(m_PropertiesFontSize, SIGNAL(valueChanged(int)), this,
          SLOT(propertiesFontSizeChanged(int)));
  m_PropertiesFontCharSet = new UndoStackDisabler<QLineEdit>(this);
  m_PropertiesFontCharSet->setUndoStack(m_MainWindow->undoStack());
  addLabeledWidget(this, fontPropsLayout, tr("Charset: "),
                   m_PropertiesFontCharSet);
  connect(m_PropertiesFontCharSet, SIGNAL(editingFinished()), this,
          SLOT(propertiesFontCharSetChanged()));
  ...
  addLabeledWidget(this, fontPropsLayout, tr("First character: "),
                   m_PropertiesFontOffset);
  connect(m_PropertiesFontOffset, SIGNAL(valueChanged(int)), this,
          SLOT(propertiesFontOffsetChanged(int)));
  ...
  connect(m_PropertiesRawStart, SIGNAL(valueChanged(int)), this,
          SLOT(propertiesRawStartChanged(int)));
  m_PropertiesRawLength = new UndoStackDisabler<QSpinBox>(this);
  m_PropertiesRawLength->setMinimum(0);
  m_PropertiesRawLength->setMaximum(
  ...
  connect(m_PropertiesRawLength, SIGNAL(valueChanged(int)), this,
          SLOT(propertiesRawLengthChanged(int)));
  ...
  m_PropertiesMemoryAddress->setMaximum(
      addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_G_END) - 4);
  m_PropertiesMemoryAddress->setSingleStep(4);
  m_PropertiesMemoryAddress->setKeyboardTracking(false);
  addLabeledWidget(this, propMemLayout, tr("Address: "),
                   m_PropertiesMemoryAddress);
  connect(m_PropertiesMemoryAddress, SIGNAL(valueChanged(int)), this,
          SLOT(propertiesMemoryAddressChanged(int)));
  m_PropertiesMemoryLoaded = new QCheckBox(this);
  addLabeledWidget(this, propMemLayout, tr("Loaded: "),
                   m_PropertiesMemoryLoaded);
  connect(m_PropertiesMemoryLoaded, SIGNAL(stateChanged(int)), this,
          SLOT(propertiesMemoryLoadedChanged(int)));
  // QFrame* line = new QFrame();
  // line->setFrameShape(QFrame::HLine);
  // line->setFrameShadow(QFrame::Sunken);
  // propMemLayout->addWidget(line);
  // TODO: New groupbox Storage Options: Compressed:, Embedded:, Flash:,
  // Address: m_PropertiesDataEmbedded = new QCheckBox(this);
  // addLabeledWidget(this, propMemLayout, tr("Embedded: "),
  // m_PropertiesDataEmbedded); connect(m_PropertiesDataEmbedded,
  // SIGNAL(stateChanged(int)), this, SLOT(propertiesDataEmbeddedChanged(int)));
  ...
  connect(m_PropertiesDataStorage, SIGNAL(currentIndexChanged(int)), this,
          SLOT(propertiesDataStorageChanged(int)));
  m_PropertiesDataCompressed = new QCheckBox(this);
  addLabeledWidget(this, propDataLayout, tr("Compressed: "),
                   m_PropertiesDataCompressed);
  connect(m_PropertiesDataCompressed, SIGNAL(stateChanged(int)), this,
          SLOT(propertiesDataCompressedChanged(int)));
  ...
  m_PropertiesFlashAddressLabel = addLabeledWidget(
      this, propDataLayout, tr("Flash Address: "), m_PropertiesFlashAddress);
  connect(m_PropertiesFlashAddress, SIGNAL(valueChanged(int)), this,
          SLOT(propertiesFlashAddressChanged(int)));
  m_PropertiesData->setLayout(propDataLayout);

  QVBoxLayout *helpLayout = new QVBoxLayout();
  m_HelpfulLabel = new ContentLabel(m_ContentList);
  connect(m_HelpfulLabel, &ContentLabel::contentDropped, m_ContentList,
          &ContentTreeWidget::contentDropped);
  m_HelpfulLabel->setWordWrap(true);
  m_HelpfulLabel->setText(
      tr("<i>No content has been added to the project yet.<br><br>Add new "
         "content to this project to automatically convert it to a hardware "
         "compatible format.</i>"));
...
ContentManager::~ContentManager() {
  ...
  for (QTreeWidgetItemIterator it(m_ContentList); *it; ++it) {
    ContentInfo *info =
        (ContentInfo *)(void *)(*it)->data(0, Qt::UserRole).value<quintptr>();
    delete info;
  }
}
...
void ContentManager::bindCurrentDevice() {
  ...
  for (int i = 0; i < g_ImageFormatIntfNb[FTEDITOR_CURRENT_DEVICE]; ++i) {
    QString format =
        g_BitmapFormatToString[FTEDITOR_CURRENT_DEVICE]
                              [g_ImageFormatFromIntf[FTEDITOR_CURRENT_DEVICE]
                                                    [i]];
  ...
  for (int i = 0; i < g_FontFormatIntfNb[FTEDITOR_CURRENT_DEVICE]; ++i) {
    QString format =
        g_BitmapFormatToString[FTEDITOR_CURRENT_DEVICE]
                              [g_FontFormatFromIntf[FTEDITOR_CURRENT_DEVICE]
                                                   [i]];
    if (format.startsWith("COMPRESSED_RGBA_"))
      format.replace("COMPRESSED_RGBA_", "");
  ...
  if (info) m_ContentList->setCurrentItem(info->View);
...
class ContentManager::Add : public QUndoCommand {
 public:
  ...
  virtual void undo() {
    ...
    for (int i = 0; i < m_ContentManager->contentList()->topLevelItemCount();
         ++i) {
      item = m_ContentManager->contentList()->topLevelItem(i);
      info =
          (ContentInfo *)(void *)item->data(0, Qt::UserRole).value<quintptr>();
      if (m_ContentInfo != info &&
          m_ContentInfo->SourcePath == info->SourcePath) {
        keepThisResource = true;
      }
    }

    if (!keepThisResource) {
      QFile::rename(m_ContentInfo->SourcePath,
                    m_ContentInfo->SourcePath + ".rem");
    }
  ...
  virtual void redo() {
    ...
    QFile::rename(m_ContentInfo->SourcePath + ".rem",
                  m_ContentInfo->SourcePath);
...
ContentInfo *ContentManager::add(const QString &filePath) {
  ...
  auto updateFlashSize = [&]() {
    ...
    while ((size_t)contentInfo->FlashAddress + contentSize > globalSize &&
           newIndex < projectFlash->count()) {
...
class ContentManager::Remove : public QUndoCommand {
 public:
  ...
  virtual void redo() {
    ...
    if (!m_WhenCloseProject)
    {
            ...
            bool keepThisResource = false;
            for (int i = 0; i <
    m_ContentManager->contentList()->topLevelItemCount(); ++i)
            {
                    item = m_ContentManager->contentList()->topLevelItem(i);
                    info = (ContentInfo *)(void *)item->data(0,
    Qt::UserRole).value<quintptr>(); if (m_ContentInfo != info &&
    m_ContentInfo->SourcePath == info->SourcePath)
                    {
                            keepThisResource = true;
                    }
 ...
 private:
  ContentManager *m_ContentManager;
  // int m_Index; // FIXME: When undo-ing, the item will appear at the end
  // rather than it's original index.
  ContentInfo *m_ContentInfo;
  ...
  for (QTreeWidgetItemIterator it(m_ContentList); *it; ++it) {
    ...
    if (info->MemoryLoaded) {
      ...
      if (size > 0) {
        ...
        if (nextAddress > freeAddress) freeAddress = nextAddress;
      }
    }
  }
  if (freeAddress % 4) {
...
bool ContentManager::loadFlashMap(QString flashMapPath) {
  ...
  if (!flashMapPath.isEmpty()) {
    ...
    QString filePath = flashMapFileInfo.absolutePath() + "/" +
                       flashMapFileInfo.completeBaseName() + ".bin";

    // Load flash map
    // C:\sync_projects_work\ft800emu\bt815\paul\FULL.map
    const FlashMapInfo &flashMapInfo =
        AssetConverter::parseFlashMap(flashMapPath);
    ...
    for (QTreeWidgetItemIterator it(m_ContentList); *it; ++it) {
      ...
      if (info->Converter == ContentInfo::FlashMap) {
        QString otherAbsoluteMapPath =
            QFileInfo(info->SourcePath).absoluteFilePath();
    ...
    for (FlashMapInfo::const_iterator it(flashMapInfo.begin()),
         end(flashMapInfo.end());
         it != end; ++it) {
      std::map<QString, ContentInfo *>::iterator update =
          updateEntries.find(it->second.Name);
    ...
    if (success) {
      for (std::vector<ContentInfo *>::iterator it(removeEntries.begin()),
           end(removeEntries.end());
           it != end; ++it) {
        // Remove it
        remove(*it);
      }
...
void ContentManager::addInternal(ContentInfo *contentInfo) {
  ...
  if (contentInfo->Converter == ContentInfo::FlashMap) {
    ...
    m_MainWindow->setFlashFileNameToLabel(
        QDir::current().absoluteFilePath(m_FlashFileName));
  }
  ...
  view->setData(0, Qt::UserRole,
                QVariant::fromValue((quintptr)(void *)contentInfo));
...
void ContentManager::removeInternal(ContentInfo *contentInfo) {
  ...
  lockContent();
  if (m_ContentUploadMemoryDirty.find(contentInfo) !=
      m_ContentUploadMemoryDirty.end())
    m_ContentUploadMemoryDirty.erase(contentInfo);
  if (m_ContentUploadFlashDirty.find(contentInfo) !=
      m_ContentUploadFlashDirty.end())
...
void ContentManager::add() {
  ...
  QStringList fileNameList = QFileDialog::getOpenFileNames(
      this, tr("Add Content"), saveDirPath, tr("All files (*.*)"));
...
void ContentManager::copyFlashFile() {
  if (m_MainWindow->isProjectSaved()) {
    ...
    // copy file .map
    QFile::copy(m_FlashFileName,
                projectFlashFolder + QFileInfo(m_FlashFileName).fileName());

    // copy file .bin
    QString binFileName(m_FlashFileName);
    binFileName.replace(m_FlashFileName.length() - 3, 3, "bin");
    QFile::copy(binFileName,
                projectFlashFolder + QFileInfo(binFileName).fileName());
...
void ContentManager::importFlashMapped() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Import Mapped Flash Image"), m_MainWindow->getFileDialogPath(),
      tr("Flash image map (*.map)"));

  if (fileName.isEmpty()) return;

  // check flash size
  if (!m_MainWindow->checkAndPromptFlashPath(fileName)) return;

  m_FlashFileName = fileName;

  // check if flash files exist
  QString checkFlashPath =
      QDir::currentPath() + "/flash/" + QFileInfo(m_FlashFileName).baseName();
  int answer = -1;
  if (!QFile::exists(checkFlashPath + ".bin") ||
      !QFile::exists(checkFlashPath + ".map")) {
    // ask if user wanna move flash files
    answer = QMessageBox::question(
        this, tr("Copy flash files"),
        tr("Do you want to copy flash files to project folder?"),
  ...
  QString relativeFlashMapPath =
      QDir(QDir::currentPath()).relativeFilePath(m_FlashFileName);
  if (!loadFlashMap(relativeFlashMapPath)) {
    QMessageBox::critical(this, tr("Import Mapped Flash Image"),
                          tr("Unable to import mapped flash image"));
    findFlashMapPath(true);
  }
}
...
ContentInfo *ContentManager::find(const QString &destName) {
  if (destName.isEmpty()) return NULL;

  for (QTreeWidgetItemIterator it(m_ContentList); *it; ++it) {
    ContentInfo *info =
        (ContentInfo *)(void *)(*it)->data(0, Qt::UserRole).value<quintptr>();
    if (info->DestName == destName) return info;
  }
...
void ContentManager::suppressOverlapCheck() { ++m_OverlapSuppressed; }
...
bool ContentManager::cacheImageInfo(ContentInfo *info) {
  if (info->CachedImage) return true;
  ImageInfo bitmapInfo;
  if (!AssetConverter::getImageInfo(bitmapInfo, info->DestName)) return false;
...
int ContentManager::getContentSize(ContentInfo *contentInfo) {
  ...
  if (contentInfo->Converter == ContentInfo::FlashMap)
  {
          // In future, if useful, may want special handling for a few file
  extensions that can load through the coprocessor (jpg, jpeg, png, bin vs raw)
...
int ContentManager::getFlashSize(ContentInfo *contentInfo) {
  ...
  /*
  if (contentInfo->Converter == ContentInfo::Image || contentInfo->Converter ==
  ContentInfo::Font)
  {
          switch (contentInfo->ImageFormat)
          {
...
void ContentManager::clear(bool whenCloseProject) {
  ...
  // Iterate through the list and copy, because we shouldn't delete while
  // iterating
  getContentInfos(contentInfos);
  // Delete all
  m_MainWindow->undoStack()->beginMacro(tr("Clear content"));
  for (std::vector<ContentInfo *>::iterator it = contentInfos.begin(),
                                            end = contentInfos.end();
       it != end; ++it) {
  ...
  if (whenCloseProject)
  {
          QDir removeDir(QDir::currentPath() + '/' +
  ContentManager::ResourceDir); for each (QString path in
  removeDir.entryList(QDir::Files))
          {
                  QRegularExpression re("\"sourcePath\": \".+" +
  QFileInfo(path).completeBaseName()); if (projectContent.indexOf(re) == -1)
                  {
                          QFile::remove(removeDir.path() + '/' + path);
                  }
                  else if (path.endsWith(".rem"))
                  {
                          // Remove extension "rem" because resource file is
  still in use QFile::rename(removeDir.path() + '/' + path, removeDir.path() +
...
void ContentManager::selectionChanged(QTreeWidgetItem *current,
  ...
  if (current) {
    ...
    if (!m_ContentList->currentItem())
      m_ContentList->setCurrentItem(
          ((ContentInfo *)(void *)current->data(0, Qt::UserRole)
               .value<quintptr>())
              ->View);
    rebuildGUIInternal((ContentInfo *)(void *)current->data(0, Qt::UserRole)
                           .value<quintptr>());
  } else {
  ...
  if (!contentInfo->BuildError.isEmpty()) {
    ...
    if ((contentInfo->MemoryLoaded && contentInfo->OverlapMemoryFlag) ||
      ...
      if (contentInfo->OverlapFlashFlag) {
        ...
        } else if ((size_t)contentInfo->FlashAddress + contentSize >
                   globalSize)  // TODO: Maybe cache getFlashSize
        {
          text = "No Space";
...
void ContentManager::rebuildGUIInternal(ContentInfo *contentInfo) {
  ...
  m_PropertiesCommonSourceFile->setToolTip(
      QDir(m_PropertiesCommonSourceFile->text()).absolutePath());
  ...
  switch (contentInfo->Converter) {
    case ContentInfo::Image:
      m_PropertiesImageFormat->setCurrentIndex(
          g_ImageFormatToIntf[FTEDITOR_CURRENT_DEVICE]
                             [contentInfo->ImageFormat %
    ...
    case ContentInfo::Raw:
      ...
      break;
    case ContentInfo::Font:
      m_PropertiesFontFormat->setCurrentIndex(
          g_FontFormatToIntf[FTEDITOR_CURRENT_DEVICE]
                            [contentInfo->ImageFormat %
                             g_BitmapFormatEnumNb[FTEDITOR_CURRENT_DEVICE]]);
      m_PropertiesFontSize->setValue(contentInfo->FontSize);
      m_PropertiesFontCharSet->setText(
          contentInfo->FontCharSet);  // NOTE: Number of characters up to 128;
                                      // export depends on number of characters
                                      // in charset also!
      m_PropertiesFontOffset->setValue(contentInfo->FontOffset);
      break;
    case ContentInfo::ImageCoprocessor:
      m_PropertiesImageMono->setCheckState(
          contentInfo->ImageMono ? Qt::Checked : Qt::Unchecked);
      break;
  }
  if (contentInfo->Converter != ContentInfo::Invalid) {
    m_PropertiesMemoryLoaded->setCheckState(
        contentInfo->MemoryLoaded ? Qt::Checked : Qt::Unchecked);
    int memSize = addr(FTEDITOR_CURRENT_DEVICE, FTEDITOR_RAM_G_END);
    ...
    m_PropertiesDataCompressed->setCheckState(
        contentInfo->DataCompressed ? Qt::Checked : Qt::Unchecked);
    if (g_Flash) {
  ...
  if (contentInfo->Converter == ContentInfo::Invalid) {
    ...
    m_PropertiesData->setHidden(true);
    props->setInfo(
        tr("Select a <b>Converter</b> to be used for this file. Converted "
           "files will be stored in the folder where the project is "
           "saved.<br><br><b>Image</b>: Converts an image to one of the "
           "supported formats.<br><b>Raw</b>: Does a direct binary "
           "copy.<br><b>Raw JPEG</b>: Does a raw binary copy and decodes the "
           "JPEG on the coprocessor."));
  } else {
    QString propInfo;
    if (!contentInfo->BuildError.isEmpty()) {
    ...
    m_PropertiesDataCompressed->setDisabled(
    ...
    switch (contentInfo->Converter) {
      case ContentInfo::Image: {
        ...
        QPixmap pixmap;
        bool loadSuccess =
            pixmap.load(contentInfo->DestName + "_converted-fs8.png") ||
            pixmap.load(contentInfo->DestName + "_converted.png");
        m_PropertiesImagePreview->setHidden(!loadSuccess);
        if (loadSuccess) {
          m_PropertiesImageLabel->setPixmap(pixmap.scaled(
              m_PropertiesImageLabel->width(), m_PropertiesImageLabel->width(),
              Qt::KeepAspectRatio));
        ...
        if (contentInfo->BuildError.isEmpty()) {
          ...
          propInfo +=
              tr("<b>Size: </b> ") + QString::number(rawInfo.size()) + " bytes";
          propInfo += tr("<br><b>Compressed: </b> ") +
                      QString::number(binInfo.size()) + " bytes";
          /*if (loadSuccess)
                  {
                          // stride etc
                          FT800EMU::BitmapInfo bitmapInfo;
                          if (AssetConverter::getImageInfo(bitmapInfo,
             contentInfo->DestName))
                          {
                                  propInfo += tr("<br><b>Width: </b> ") +
             QString::number(bitmapInfo.LayoutWidth); propInfo +=
             tr("<br><b>Height: </b> ") +
             QString::number(bitmapInfo.LayoutHeight); propInfo +=
             tr("<br><b>Format: </b> "); switch (bitmapInfo.LayoutFormat)
                                  {
                                          case ARGB1555: propInfo += "ARGB1555";
             break; case L1: propInfo += "L1"; break; case L4: propInfo += "L4";
             break; case L8: propInfo += "L8"; break; case RGB332: propInfo +=
             "RGB332"; break; case ARGB2: propInfo += "ARGB2"; break; case
             ARGB4: propInfo += "ARGB4"; break; case RGB565: propInfo +=
             "RGB565"; break; case PALETTED: propInfo += "PALETTED"; break; case
             TEXT8X8: propInfo += "TEXT8X8"; break; case TEXTVGA: propInfo +=
             "TEXTVGA"; break; case BARGRAPH: propInfo += "BARGRAPH"; break;
                                  }
                                  propInfo += tr("<br><b>Stride: </b> ") +
             QString::number(bitmapInfo.LayoutStride);
                          }
                          else
                          {
                                  propInfo += tr("<br><b>Error: </b>Unable to
             load raw header.");
                          }
                  }*/
          if (cacheImageInfo(contentInfo)) {
            propInfo += tr("<br><b>Width: </b> ") +
                        QString::number(contentInfo->CachedImageWidth);
            propInfo += tr("<br><b>Height: </b> ") +
                        QString::number(contentInfo->CachedImageHeight);
            propInfo += tr("<br><b>Line Stride: </b> ") +
                        QString::number(contentInfo->CachedImageStride);
          }
        }
        break;
      }
      ...
      RawDetect : {
        ...
        if (rawFile.open(QIODevice::ReadOnly)) {
          ...
          if (signature == 0xAAAA0100) {
            ...
            quint32 num_frames;
            in >> num_frames;
            propInfo += tr("<br><b>Number of Frames: </b> ") +
                        QString::number(num_frames);
            for (quint32 i = 0; i < num_frames && i < 256; ++i) {
              quint32 nbytes, ptr;
              in >> nbytes;
              in >> ptr;
              propInfo +=
                  QString("<br><b>") + QString::number(i) + " </b> (0x" +
                  QString::number(ptr, 16).rightJustified(8, '0').toLower() +
      ...
      case ContentInfo::ImageCoprocessor: {
        ...
        m_PropertiesImagePreview->setHidden(!loadSuccess);
        m_PropertiesImageLabel->setPixmap(pixmap.scaled(
            m_PropertiesImageLabel->width() - 32,
            m_PropertiesImageLabel->width() - 32, Qt::KeepAspectRatio));
        if (loadSuccess)
          m_PropertiesImageLabel->repaint();
        else {
          if (!propInfo.isEmpty()) propInfo += "<br>";
        ...
        if (contentInfo->BuildError.isEmpty()) {
          if (cacheImageInfo(contentInfo)) {
            propInfo += tr("<br><b>Format: </b> ") +
                        bitmapFormatToString(FTEDITOR_CURRENT_DEVICE,
                                             contentInfo->ImageFormat);
            propInfo += tr("<br><b>Width: </b> ") +
                        QString::number(contentInfo->CachedImageWidth);
            propInfo += tr("<br><b>Height: </b> ") +
      ...
      case ContentInfo::FlashMap: {
        ...
        if (contentInfo->BuildError.isEmpty()) {
          QFileInfo rawInfo(contentInfo->DestName + ".raw");
          if (rawInfo.exists())
            propInfo += tr("<br><b>Size: </b> ") +
                        QString::number(rawInfo.size()) + " bytes";
          goto RawDetect;
        }
        break;
      }
  ...
  if (contentInfo->Converter != ContentInfo::Invalid) {
    ...
    if (!srcExists) {
      contentInfo->BuildError =
          "<i>(Filesystem)</i><br>Source file does not exist.";
      contentInfo->ExternalDirty = true;
      ...
      if (contentInfo->BuildError.isEmpty()) {
        if (metaExists) {
          ...
          if (srcInfo.lastModified() > metaInfo.lastModified()) {
            // FIXME: Perhaps better to store the srcInfo.lastModified() in the
            // meta and compare if equal in case of swapping with older files
            ...
            QString srcBinFile = srcInfo.absolutePath() + "/" +
                                 srcInfo.completeBaseName() + ".bin";
            bool srcBinExists = QFile::exists(srcBinFile);
            if (!srcBinExists) {
              contentInfo->BuildError =
                  "<i>(Filesystem)</i><br>Flash binary file does not exist.";
              contentInfo->ExternalDirty = true;
              goto SkipRebuild;  // Error, skip rebuild
            } else {
              QFileInfo srcBinInfo(srcBinFile);
              if (srcBinInfo.lastModified() > metaInfo.lastModified()) {
                // FIXME: Perhaps better to store the srcInfo.lastModified() in
                // the meta and compare if equal in case of swapping with older
                // files
                printf(
                    "Flash binary file has been modified, ignore meta, "
                    "rebuild\n");
                goto IgnoreMeta;  // Ignore meta comparison and rebuild
              }
            }
          }
          // else goto CompareMeta; // Implicit // Ignore meta comparison and
          // rebuild CompareMeta:
          ;
          {
            QFile file(metaFile);
      ...
      if (equalMeta) {
        ...
        if (contentInfo->DestName.contains('/')) {
          QString destDir = contentInfo->DestName.left(
              contentInfo->DestName.lastIndexOf('/'));
          if (!QDir(QDir::currentPath()).mkpath(destDir)) {
            contentInfo->BuildError =
                "<i>(Filesystem)</i><br>Unable to create destination path.";
          }
        }
        if (contentInfo->BuildError.isEmpty()) {
          // Run convertor
          switch (contentInfo->Converter) {
            case ContentInfo::Image:
              AssetConverter::convertImage(
                  contentInfo->BuildError, contentInfo->SourcePath,
                  contentInfo->DestName, contentInfo->ImageFormat);
              break;
            case ContentInfo::Raw:
              AssetConverter::convertRaw(
                  contentInfo->BuildError, contentInfo->SourcePath,
                  contentInfo->DestName, contentInfo->RawStart,
                  contentInfo->RawLength);
              break;
            case ContentInfo::ImageCoprocessor:
              AssetConverter::convertImageCoprocessor(
                  contentInfo->BuildError, contentInfo->SourcePath,
                  contentInfo->DestName, contentInfo->ImageMono, true,
                  FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810,
                  contentInfo->PalettedAddress);
              break;
            case ContentInfo::Font:
              AssetConverter::convertFont(
                  contentInfo->BuildError, contentInfo->SourcePath,
                  contentInfo->DestName, contentInfo->ImageFormat,
                  contentInfo->FontSize, contentInfo->FontCharSet,
                  contentInfo->FontOffset);
              break;
            case ContentInfo::FlashMap:
              AssetConverter::convertFlashMap(
                  contentInfo->BuildError, contentInfo->SourcePath,
                  contentInfo->DestName, contentInfo->MappedName);
              break;
            default:
              contentInfo->BuildError =
                  "<i>(Critical Error)</i><br>Unknown converter selected.";
              break;
          }
        }
        if (contentInfo->BuildError.isEmpty()) {
...
void ContentManager::reuploadInternal(ContentInfo *contentInfo, bool memory,
                                      bool flash) {
  ...
  if (contentInfo->Converter != ContentInfo::Invalid &&
      ((contentInfo->MemoryLoaded && memory) ||
       ((contentInfo->DataStorage == ContentInfo::Flash) && flash))) {
    if (contentInfo->Converter == ContentInfo::Image ||
        contentInfo->Converter == ContentInfo::Font) {
      // Bitmap setup is always updated after upload and requires cached image
      // info
...
void ContentManager::recalculateOverlapMemoryInternal() {
  ...
  for (QTreeWidgetItemIterator left(m_ContentList); *left;) {
    ContentInfo *leftInfo =
        (ContentInfo *)(void *)(*left)->data(0, Qt::UserRole).value<quintptr>();
    if (leftInfo->Converter != ContentInfo::Invalid && leftInfo->MemoryLoaded) {
      ...
      if (leftSize >= 0) {
        ...
        if (leftAddr + leftSize > globalSize) {
          ...
          if (m_ContentOverlapMemory.find(leftInfo) ==
              m_ContentOverlapMemory.end())
            m_ContentOverlapMemory.insert(leftInfo);
          ++left;
        } else {
          globalUsage += leftSize;
          for (QTreeWidgetItemIterator right(++left); *right; ++right) {
            ContentInfo *rightInfo = (ContentInfo *)(void *)(*right)
                                         ->data(0, Qt::UserRole)
                                         .value<quintptr>();
            if (rightInfo->Converter != ContentInfo::Invalid &&
                rightInfo->MemoryLoaded) {
                ...
                if (leftAddr < rightAddr) {
                  if (leftAddr + leftSize > rightAddr) {
                    ...
                    printf("CM: Content '%s' overlap with '%s'\n",
                           leftInfo->DestName.toLocal8Bit().data(),
                           rightInfo->DestName.toLocal8Bit().data());
                    if (m_ContentOverlapMemory.find(leftInfo) ==
                        m_ContentOverlapMemory.end())
                      m_ContentOverlapMemory.insert(leftInfo);
                    if (m_ContentOverlapMemory.find(rightInfo) ==
                        m_ContentOverlapMemory.end())
  ...
  for (std::set<ContentInfo *>::iterator it(contentOverlap.begin()),
       end(contentOverlap.end());
       it != end; ++it) {
  ...
  for (std::set<ContentInfo *>::iterator it(m_ContentOverlapMemory.begin()),
       end(m_ContentOverlapMemory.end());
       it != end; ++it) {
...
void ContentManager::recalculateOverlapFlashInternal() {
  ...
  for (QTreeWidgetItemIterator left(m_ContentList); *left;) {
    ContentInfo *leftInfo =
        (ContentInfo *)(void *)(*left)->data(0, Qt::UserRole).value<quintptr>();
    if (leftInfo->Converter != ContentInfo::Invalid &&
        (leftInfo->DataStorage == ContentInfo::Flash)) {
      int leftSize = getContentSize(leftInfo);
      // printf("leftSize: %i\n", leftSize);
      if (leftSize >= 0) {
        ...
        if (leftAddr < FTEDITOR_FLASH_FIRMWARE_SIZE) {
          printf("CM: Content '%s' overlaps with flash firmware\n",
                 leftInfo->DestName.toLocal8Bit().data());
          if (m_ContentOverlapFlash.find(leftInfo) ==
              m_ContentOverlapFlash.end())
        ...
        if ((size_t)leftAddr + leftSize > globalSize) {
          ...
          for (QTreeWidgetItemIterator right(++left); *right; ++right) {
            ContentInfo *rightInfo = (ContentInfo *)(void *)(*right)
                                         ...
                                         .value<quintptr>();
            if (rightInfo->Converter != ContentInfo::Invalid &&
                (rightInfo->DataStorage == ContentInfo::Flash)) {
              int rightSize = getContentSize(rightInfo);
              if (rightSize >= 0) {
                ...
                if (leftAddr < rightAddr) {
                  if (leftAddr + leftSize > rightAddr) {
                    ...
                    if (m_ContentOverlapFlash.find(leftInfo) ==
                        m_ContentOverlapFlash.end())
                      m_ContentOverlapFlash.insert(leftInfo);
                    if (m_ContentOverlapFlash.find(rightInfo) ==
                        m_ContentOverlapFlash.end())
  ...
  for (std::set<ContentInfo *>::iterator it(m_ContentOverlapFlash.begin()),
       end(m_ContentOverlapFlash.end());
       it != end; ++it) {
    if (contentOverlap.find(*it) == contentOverlap.end()) {
      // Newly overlapping content
      printf("CM: Content '%s' is overlapping\n",
...
ContentInfo *ContentManager::current() {
  if (!m_ContentList->currentItem()) return NULL;
  return (ContentInfo *)(void *)m_ContentList->currentItem()
      ->data(0, Qt::UserRole)
      .value<quintptr>();
}
...
std::vector<ContentInfo *> ContentManager::allRam() {
  ...
  for (QTreeWidgetItemIterator it(m_ContentList); *it; ++it) {
    ContentInfo *info =
        (ContentInfo *)(void *)(*it)->data(0, Qt::UserRole).value<quintptr>();
    if (info->MemoryLoaded) res.push_back(info);
  }
  return res;
}
...
std::vector<ContentInfo *> ContentManager::allFlash() {
  ...
  for (QTreeWidgetItemIterator it(m_ContentList); *it; ++it) {
    ContentInfo *info =
        (ContentInfo *)(void *)(*it)->data(0, Qt::UserRole).value<quintptr>();
    if (info->DataStorage == ContentInfo::Flash) res.push_back(info);
  }
  return res;
}
...
int ContentManager::editorFindHandle(ContentInfo *contentInfo,
                                     DlEditor *dlEditor) {
  int handle = -1;
        ...
        if (parsed.IdLeft == FTEDITOR_CO_COMMAND) {
          switch (parsed.IdRight) {
            case CMD_SETBITMAP & 0xFF:
              if (parsed.Parameter[0].U == contentInfo->bitmapAddress() &&
                  handle != -1)
                return handle;
              break;
...
int ContentManager::editorFindFreeHandle(DlEditor *dlEditor) {
  ...
  for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
    ...
    if (parsed.ValidId && parsed.IdLeft == 0 &&
        ...
        parsed.Parameter[0].U < BITMAP_SETUP_HANDLES_NB) {
      ...
      if (parsed.ValidId && parsed.IdLeft == FTEDITOR_CO_COMMAND &&
          parsed.IdRight == (CMD_SETFONT2 & 0xFF) &&
          parsed.Parameter[0].U < BITMAP_SETUP_HANDLES_NB) {
        handles[parsed.Parameter[0].U] = true;
      } else if (parsed.ValidId && parsed.IdLeft == FTEDITOR_CO_COMMAND &&
                 parsed.IdRight == (CMD_SETSCRATCH & 0xFF) &&
                 parsed.Parameter[0].U < BITMAP_SETUP_HANDLES_NB) {
        handles[parsed.Parameter[0].U] = true;
      } else if (parsed.ValidId && parsed.IdLeft == FTEDITOR_CO_COMMAND &&
                 parsed.IdRight == (CMD_ROMFONT & 0xFF) &&
                 parsed.Parameter[0].U < BITMAP_SETUP_HANDLES_NB) {
        handles[parsed.Parameter[0].U] = true;
      }
    }
...
void ContentManager::editorUpdateHandle(ContentInfo *contentInfo,
                                        DlEditor *dlEditor, bool updateSize) {
  int handleLine = -1;
    ...
    if (parsed.ValidId) {
      if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810) {
        if (parsed.IdLeft == FTEDITOR_CO_COMMAND) {
          switch (parsed.IdRight) {
            ...
            case CMD_SETFONT2 & 0xFF: {
              // This is really not a useful case, but it can happen, so support
              // it...
              handleLine = i;
      ...
      switch (parsed.IdRight) {
        ...
        /* case FTEDITOR_DL_PALETTE_SOURCE:
                        if (addressOk && parsed.Parameter[0].U ==
  ...
  if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810) {
    if (!cmdSetBitmap)  // User friendly fixup for _H, does not work with bitmap
                        // handle reuse, but not a critical use case
    {
      if (layoutLine >= 0 && layoutHLine < 0) {
        ...
        if (sizeLine > layoutLine) ++sizeLine;
        if (sizeHLine > layoutLine) ++sizeHLine;
      }
    ...
    /* if (insertOkLine >= 0 && paletteAddrLine < 0 &&
...
void ContentManager::editorUpdatePaletteAddress(int newAddr, int oldAddr,
                                                DlEditor *dlEditor) {
  for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
    const DlParsed &parsed = dlEditor->getLine(i);
    if (parsed.ValidId && parsed.IdLeft == 0 &&
        parsed.IdRight == FTEDITOR_DL_PALETTE_SOURCE &&
        (parsed.Parameter[0].I & ~3) == oldAddr) {
      DlParsed pa = parsed;
...
void ContentManager::editorUpdateHandleAddress(int newAddr, int oldAddr,
                                               DlEditor *dlEditor) {
  for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
    const DlParsed &parsed = dlEditor->getLine(i);
    if (parsed.ValidId && parsed.IdLeft == 0 &&
        parsed.IdRight == FTEDITOR_DL_BITMAP_SOURCE &&
        parsed.Parameter[0].I == oldAddr) {
      DlParsed pa = parsed;
      pa.Parameter[0].I = newAddr;
      dlEditor->replaceLine(i, pa);
    } else if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810) {
      if (parsed.ValidId && parsed.IdLeft == FTEDITOR_CO_COMMAND &&
          parsed.IdRight == (CMD_SETBITMAP & 0xFF) &&
          parsed.Parameter[0].I == oldAddr) {
        DlParsed pa = parsed;
        pa.Parameter[0].I = newAddr;
        dlEditor->replaceLine(i, pa);
...
// Update font adress
void ContentManager::editorUpdateFontAddress(int newAddr, int oldAddr,
                                             DlEditor *dlEditor) {
  for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
    const DlParsed &parsed = dlEditor->getLine(i);
    if (parsed.ValidId && parsed.IdLeft == FTEDITOR_CO_COMMAND &&
        (parsed.IdRight == (CMD_SETFONT & 0xFF) ||
         parsed.IdRight == (CMD_SETFONT2 & 0xFF)) &&
        parsed.Parameter[1].I == oldAddr) {
      DlParsed pa = parsed;
...
void ContentManager::editorUpdateFontOffset(ContentInfo *contentInfo,
                                            DlEditor *dlEditor) {
  for (int i = 0; i < FTEDITOR_DL_SIZE; ++i) {
    const DlParsed &parsed = dlEditor->getLine(i);
    if (parsed.ValidId && parsed.IdLeft == FTEDITOR_CO_COMMAND &&
        parsed.IdRight == (CMD_SETFONT2 & 0xFF) &&
        parsed.Parameter[1].I == contentInfo->MemoryAddress) {
      DlParsed pa = parsed;
...
void editorPurgePalette8(DlEditor *dlEditor, int &line) {
  ...
  if (line > 0) {
    ...
    for (; line >= 0; --line) {
      ...
      if (parsed.IdLeft != FTEDITOR_DL_INSTRUCTION) break;
  ...
  if (line > 0 && line + 1 < FTEDITOR_DL_SIZE &&
      line + 1 < dlEditor->getLineCount()) {
    const DlParsed &prev = dlEditor->getLine(line - 1);
    const DlParsed &next = dlEditor->getLine(line + 1);
    if (prev.ValidId && prev.IdLeft == FTEDITOR_DL_INSTRUCTION &&
        prev.IdRight == FTEDITOR_DL_SAVE_CONTEXT && next.ValidId &&
        next.IdLeft == FTEDITOR_DL_INSTRUCTION &&
        next.IdRight == FTEDITOR_DL_RESTORE_CONTEXT) {
...
void tempBeginIdel(CodeEditor *dlEditor) {
  static_cast<DlEditor *>(dlEditor->parent())
      ->mainWindow()
      ->undoStack()
      ->beginMacro("Interactive Delete");
}
...
void editorProcessPaletteSource(int bitmapAddress, int paletteAddress,
                                DlEditor *dlEditor) {
    ...
    switch (parsed.IdRight) {
      case FTEDITOR_DL_PALETTE_SOURCE:
        if ((parsed.Parameter[0].I & ~3) == paletteAddress) {
          if (process ==
              FTEDITOR_PURGE_PALETTE_SOURCE)  // BUG: May remove too many
          {
            dlEditor->removeLine(i);
            --i;
          }
        ...
        break;
      case FTEDITOR_DL_BITMAP_HANDLE:
        if (parsed.Parameter[0].I < 32) curBitmapHandle = parsed.Parameter[0].I;
        break;
      case FTEDITOR_DL_BITMAP_SOURCE:
        bitmapHandle[curBitmapHandle] = parsed.Parameter[0].U == bitmapAddress;
        break;
...
void ContentManager::editorRemoveContent(ContentInfo *contentInfo,
                                         DlEditor *dlEditor) {
  if (requirePaletteAddress(contentInfo)) {
    if (contentInfo->ImageFormat == PALETTED8)
      editorProcessPaletteSource<FTEDITOR_PURGE_PALETTE_SOURCE_8>(
          contentInfo->bitmapAddress(), contentInfo->MemoryAddress,
          m_MainWindow->dlEditor()),
          editorProcessPaletteSource<FTEDITOR_PURGE_PALETTE_SOURCE_8>(
              contentInfo->bitmapAddress(), contentInfo->MemoryAddress,
              m_MainWindow->cmdEditor());
    else
      editorProcessPaletteSource<FTEDITOR_PURGE_PALETTE_SOURCE>(
          contentInfo->bitmapAddress(), contentInfo->MemoryAddress,
          m_MainWindow->dlEditor()),
          editorProcessPaletteSource<FTEDITOR_PURGE_PALETTE_SOURCE>(
              contentInfo->bitmapAddress(), contentInfo->MemoryAddress,
  ...
  if (bitmapHandle >= 0) {
    ...
    for (int i = bitmapLine;
         i < FTEDITOR_DL_SIZE && i < dlEditor->getLineCount(); ++i) {
      const DlParsed &parsed = dlEditor->getLine(i);
      if (parsed.ValidId) {
        if (drawingBitmaps)  // ISSUE#113: NOTE: If you do not want bitmaps
                             // removed, disable this block
        {
          ...
          if (removedVertex) {
            ...
            if (i < FTEDITOR_DL_SIZE && i < dlEditor->getLineCount()) {
              ...
              if (beginVertex.IdLeft == 0 &&
                  beginVertex.IdRight == FTEDITOR_DL_BEGIN &&
                  endVertex.IdLeft == 0 &&
                  endVertex.IdRight == FTEDITOR_DL_END) {
        ...
        {
          if (parsed.IdRight == (CMD_TEXT & 0xFF) &&
              parsed.Parameter[2].I == bitmapHandle) {
          ...
          if (parsed.IdRight == (CMD_SETFONT & 0xFF) &&
              parsed.Parameter[0].I == bitmapHandle) {
            // Purge fonts
            dlEditor->removeLine(i);
            --i;
            continue;
          ...
          if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810) {
            if (parsed.IdRight == (CMD_SETFONT2 & 0xFF)) {
              handleActive = (parsed.Parameter[0].I ==
                              bitmapHandle);  // This switches handle
              if (handleActive) {
                // Purge fonts
                dlEditor->removeLine(i);
                --i;
...
class ContentManager::ChangeSourcePath : public QUndoCommand {
 public:
  ChangeSourcePath(ContentManager *contentManager, ContentInfo *contentInfo,
                   const QString &value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064400; }

  virtual bool mergeWith(const QUndoCommand *command) {
    const ChangeSourcePath *c = static_cast<const ChangeSourcePath *>(command);

    if (c->m_ContentInfo != m_ContentInfo) return false;

    if (c->m_NewValue == m_OldValue) return false;
...
void ContentManager::changeSourcePath(ContentInfo *contentInfo,
                                      const QString &value) {
  // Create undo/redo
  ChangeSourcePath *changeSourcePath =
      new ChangeSourcePath(this, contentInfo, value);
  ...
  if (newBitmapAddr != oldBitmapAddr) {
    editorUpdateHandleAddress(newBitmapAddr, oldBitmapAddr,
                              m_MainWindow->dlEditor());
    editorUpdateHandleAddress(newBitmapAddr, oldBitmapAddr,
                              m_MainWindow->cmdEditor());
  }
...
void ContentManager::propertiesCommonSourcePathChanged() {
  ...
  if (current() &&
      current()->SourcePath != m_PropertiesCommonSourceFile->text())
...
void ContentManager::propertiesCommonSourcePathBrowse() {
  ...
  QString fileName = QFileDialog::getOpenFileName(this, tr("Change Content"),
...
class ContentManager::ChangeDestName : public QUndoCommand {
 public:
  ChangeDestName(ContentManager *contentManager, ContentInfo *contentInfo,
                 const QString &value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
 ...
 private:
  void renameFileExt(const QString &from, const QString &to,
                     const QString &ext) const {
    if (!QFile::rename(from + ext, to + ext)) {
      // printf("cannot rename '%s' to '%s'\n", (from +
      // ext).toLocal8Bit().data(), (to + ext).toLocal8Bit().data());
 ...
 protected:
  void renameFiles(const QString &from, const QString &to) const {
    ...
    if (to.contains('/')) {
      ...
      if (!QDir(QDir::currentPath()).mkpath(destDir)) {
        // Will fail at copy, and a rebuild will happen which will also fail
        // with mkpath failure
    ...
    for (std::vector<QString>::const_iterator it(fileExt.begin()),
         end(fileExt.end());
         it != end; ++it) {
 ...
 public:
  ...
  virtual int id() const { return 9064401; }
...
void ContentManager::changeDestName(ContentInfo *contentInfo,
                                    const QString &value) {
  // Create undo/redo
  if (contentInfo->DestName != value) {
    ChangeDestName *changeDestName =
        new ChangeDestName(this, contentInfo, createName(value));
...
void ContentManager::propertiesCommonDestNameChanged() {
  ...
  if (current()) changeDestName(current(), m_PropertiesCommonName->text());
}
...
class ContentManager::ChangeConverter : public QUndoCommand {
 public:
  ChangeConverter(ContentManager *contentManager, ContentInfo *contentInfo,
                  ContentInfo::ConverterType value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064402; }
...
void ContentManager::changeConverter(ContentInfo *contentInfo,
                                     ContentInfo::ConverterType value) {
  // Create undo/redo
  m_MainWindow->undoStack()->beginMacro(tr("Change converter"));
  ChangeConverter *changeConverter =
      new ChangeConverter(this, contentInfo, value);
...
class ContentManager::ChangeImageFormat : public QUndoCommand {
 public:
  ChangeImageFormat(ContentManager *contentManager, ContentInfo *contentInfo,
                    int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064403; }

  virtual bool mergeWith(const QUndoCommand *command) {
    const ChangeImageFormat *c =
        static_cast<const ChangeImageFormat *>(command);
...
void ContentManager::changeImageFormat(ContentInfo *contentInfo, int value) {
  ...
  ChangeImageFormat *changeImageFormat =
      new ChangeImageFormat(this, contentInfo, value);
  m_MainWindow->undoStack()->beginMacro(tr("Change image format"));
  int oldImageFormat = contentInfo->ImageFormat;
  bool oldPaletteSource =
      requirePaletteAddress(contentInfo) && oldImageFormat != PALETTED8;
  int oldBitmapAddr = contentInfo->bitmapAddress();
  m_MainWindow->undoStack()->push(changeImageFormat);
  m_MainWindow->propertiesEditor()->surpressSet(true);
  bool paletteSource = requirePaletteAddress(contentInfo) &&
                       contentInfo->ImageFormat != PALETTED8;
  if (oldImageFormat == PALETTED8)
    editorProcessPaletteSource<FTEDITOR_PURGE_PALETTE_SOURCE_8>(
        oldBitmapAddr, contentInfo->MemoryAddress, m_MainWindow->dlEditor()),
        editorProcessPaletteSource<FTEDITOR_PURGE_PALETTE_SOURCE_8>(
            oldBitmapAddr, contentInfo->MemoryAddress,
            m_MainWindow->cmdEditor());
  ...
  if (contentInfo->ImageFormat == PALETTED8)
    editorProcessPaletteSource<FTEDITOR_INSERT_PALETTE_SOURCE_8>(
        newBitmapAddr, contentInfo->MemoryAddress, m_MainWindow->dlEditor()),
        editorProcessPaletteSource<FTEDITOR_INSERT_PALETTE_SOURCE_8>(
            newBitmapAddr, contentInfo->MemoryAddress,
            m_MainWindow->cmdEditor());
  else if (oldPaletteSource != paletteSource && paletteSource)
    editorProcessPaletteSource<FTEDITOR_INSERT_PALETTE_SOURCE>(
        newBitmapAddr, contentInfo->MemoryAddress, m_MainWindow->dlEditor()),
        editorProcessPaletteSource<FTEDITOR_INSERT_PALETTE_SOURCE>(
            newBitmapAddr, contentInfo->MemoryAddress,
            m_MainWindow->cmdEditor());
...
void ContentManager::propertiesImageFormatChanged(int value) {
  ...
  value = g_ImageFormatFromIntf[FTEDITOR_CURRENT_DEVICE]
                               [value %
                                g_ImageFormatIntfNb[FTEDITOR_CURRENT_DEVICE]];

  if (current() && current()->ImageFormat != value) {
    emit busyNow(this);
    auto result = QtConcurrent::run([this, value]() {
                    qDebug() << "Converter Image - Busy: "
                             << QDateTime::currentDateTime().toString(
                                    "dd.MM.yyyy hh:mm:ss");
                    changeImageFormat(current(), value);
                  }).then([this]() {
      qDebug() << "Converter Image - Free: "
...
class ContentManager::ChangeImageMono : public QUndoCommand {
 public:
  ChangeImageMono(ContentManager *contentManager, ContentInfo *contentInfo,
                  int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
...
void ContentManager::changeImageMono(ContentInfo *contentInfo, bool value) {
  // Create undo/redo
  ChangeImageMono *changeImageMono =
      new ChangeImageMono(this, contentInfo, value);
...
class ContentManager::ChangeMemoryLoaded : public QUndoCommand {
 public:
  ChangeMemoryLoaded(ContentManager *contentManager, ContentInfo *contentInfo,
                     int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
        ...
        m_NewValue(value) {
    setText(value ? tr("Load content to memory")
                  : tr("Unload content from memory"));
  }
...
void ContentManager::changeMemoryLoaded(ContentInfo *contentInfo, bool value) {
  // Create undo/redo
  ChangeMemoryLoaded *changeMemoryLoaded =
      new ChangeMemoryLoaded(this, contentInfo, value);
...
class ContentManager::ChangeMemoryAddress : public QUndoCommand {
 public:
  ChangeMemoryAddress(ContentManager *contentManager, ContentInfo *contentInfo,
                      int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064404; }

  virtual bool mergeWith(const QUndoCommand *command) {
    const ChangeMemoryAddress *c =
        static_cast<const ChangeMemoryAddress *>(command);
...
void ContentManager::changeMemoryAddress(ContentInfo *contentInfo, int value,
  ...
  // Create undo/redo
  value &= 0x7FFFFFFC;
  ChangeMemoryAddress *changeMemoryAddress =
      new ChangeMemoryAddress(this, contentInfo, value);  // Force round to 4
  if (!internal) {
  ...
  if (!internal) {
    ...
    if (requirePaletteAddress(contentInfo)) {
      editorUpdatePaletteAddress(value, oldMemoryAddr,
                                 m_MainWindow->dlEditor());
      editorUpdatePaletteAddress(value, oldMemoryAddr,
                                 m_MainWindow->cmdEditor());
    }
...
class ContentManager::ChangeDataCompressed : public QUndoCommand {
 public:
  ChangeDataCompressed(ContentManager *contentManager, ContentInfo *contentInfo,
                       int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
        ...
        m_NewValue(value) {
    setText(value ? tr("Store data compressed")
                  : tr("Store data uncompressed"));
  }
...
void ContentManager::changeDataCompressed(ContentInfo *contentInfo,
                                          bool value) {
  // Create undo/redo
  ChangeDataCompressed *changeDataCompressed =
      new ChangeDataCompressed(this, contentInfo, value);
...
class ContentManager::ChangeDataStorage : public QUndoCommand {
 public:
  ChangeDataStorage(ContentManager *contentManager, ContentInfo *contentInfo,
                    ContentInfo::StorageType value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9076474; }

  virtual bool mergeWith(const QUndoCommand *command) {
    const ChangeDataStorage *c =
        static_cast<const ChangeDataStorage *>(command);
...
void ContentManager::changeDataStorage(ContentInfo *contentInfo,
                                       ContentInfo::StorageType value) {
  // Create undo/redo
  ChangeDataStorage *changeDataStorage =
      new ChangeDataStorage(this, contentInfo, value);
...
{
public:
        ChangeFlashLoaded(ContentManager *contentManager, ContentInfo
*contentInfo, int value) : QUndoCommand(), m_ContentManager(contentManager),
                m_ContentInfo(contentInfo),
                m_OldValue(((contentInfo->DataStorage == ContentInfo::Flash)),
                m_NewValue(value)
        {
                setText(value ? tr("Load content to flash") : tr("Unload content
from flash"));
        }
        ...
        virtual void undo()
        {
                ...
                m_ContentManager->recalculateOverlapFlashInternal();
                m_ContentManager->reloadExternal(m_ContentInfo);
                if (m_ContentManager->m_CurrentPropertiesContent ==
m_ContentInfo) m_ContentManager->rebuildGUIInternal(m_ContentInfo);
        }
...
{
        // Create undo/redo
        ChangeFlashLoaded *changeFlashLoaded = new ChangeFlashLoaded(this,
contentInfo, value); m_MainWindow->undoStack()->push(changeFlashLoaded);
}
...
class ContentManager::ChangeFlashAddress : public QUndoCommand {
 public:
  ChangeFlashAddress(ContentManager *contentManager, ContentInfo *contentInfo,
                     int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9074404; }

  virtual bool mergeWith(const QUndoCommand *command) {
    const ChangeFlashAddress *c =
        static_cast<const ChangeFlashAddress *>(command);
...
void ContentManager::changeFlashAddress(ContentInfo *contentInfo, int value,
                                        bool internal) {
  // Create undo/redo
  value &= (0x7FFFFF << 5);
  int editorAddress = 0x800000 | ((value >> 5) & 0x7FFFFF);
  ChangeFlashAddress *changeFlashAddress =
      new ChangeFlashAddress(this, contentInfo, value);  // Force round to 4
  if (!internal) {
  ...
  if (!internal) {
    m_MainWindow->propertiesEditor()->surpressSet(true);
    editorUpdateHandleAddress(editorAddress, oldEditorAddr,
                              m_MainWindow->dlEditor());
    editorUpdateHandleAddress(editorAddress, oldEditorAddr,
                              m_MainWindow->cmdEditor());
    m_ContentList->setCurrentItem(contentInfo->View);
...
class ContentManager::ChangeRawStart : public QUndoCommand {
 public:
  ChangeRawStart(ContentManager *contentManager, ContentInfo *contentInfo,
                 int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064411; }
...
void ContentManager::changeRawStart(ContentInfo *contentInfo, int value) {
  // Create undo/redo
  ChangeRawStart *changeRawStart = new ChangeRawStart(
      this, contentInfo, value & 0x7FFFFFFC);  // Force round to 4
...
class ContentManager::ChangeRawLength : public QUndoCommand {
 public:
  ChangeRawLength(ContentManager *contentManager, ContentInfo *contentInfo,
                  int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064412; }
...
void ContentManager::changeRawLength(ContentInfo *contentInfo, int value) {
  // Create undo/redo
  ChangeRawLength *changeRawLength = new ChangeRawLength(
      this, contentInfo, value & 0x7FFFFFFC);  // Force round to 4
...
class ContentManager::ChangeFontFormat : public QUndoCommand {
 public:
  ChangeFontFormat(ContentManager *contentManager, ContentInfo *contentInfo,
                   int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064413; }
...
void ContentManager::changeFontFormat(ContentInfo *contentInfo, int value) {
  // Create undo/redo
  ChangeFontFormat *changeFontFormat =
      new ChangeFontFormat(this, contentInfo, value);
...
void ContentManager::propertiesFontFormatChanged(int value) {
  ...
  value =
      g_FontFormatFromIntf[FTEDITOR_CURRENT_DEVICE]
                          [value % g_FontFormatIntfNb[FTEDITOR_CURRENT_DEVICE]];
...
class ContentManager::ChangeFontSize : public QUndoCommand {
 public:
  ChangeFontSize(ContentManager *contentManager, ContentInfo *contentInfo,
                 int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064414; }
...
class ContentManager::ChangeFontCharSet : public QUndoCommand {
 public:
  ChangeFontCharSet(ContentManager *contentManager, ContentInfo *contentInfo,
                    const QString &value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
 ...
 public:
  ...
  virtual int id() const { return 9064415; }

  virtual bool mergeWith(const QUndoCommand *command) {
    const ChangeFontCharSet *c =
        static_cast<const ChangeFontCharSet *>(command);
...
void ContentManager::changeFontCharSet(ContentInfo *contentInfo,
                                       const QString &value) {
  // Create undo/redo
  if (contentInfo->FontCharSet != value) {
    ChangeFontCharSet *changeFontCharSet =
        new ChangeFontCharSet(this, contentInfo, value);
...
void ContentManager::propertiesFontCharSetChanged() {
  ...
  if (current()) changeFontCharSet(current(), m_PropertiesFontCharSet->text());
}
...
class ContentManager::ChangeFontOffset : public QUndoCommand {
 public:
  ChangeFontOffset(ContentManager *contentManager, ContentInfo *contentInfo,
                   int value)
      : QUndoCommand(),
        m_ContentManager(contentManager),
        m_ContentInfo(contentInfo),
  ...
  virtual int id() const { return 9064416; }
...
void ContentManager::changeFontOffset(ContentInfo *contentInfo, int value) {
  // Create undo/redo
  ChangeFontOffset *changeFontOffset =
      new ChangeFontOffset(this, contentInfo, value);
  m_MainWindow->undoStack()->beginMacro(tr("Change font size"));
  int oldBitmapAddr = contentInfo->bitmapAddress();
...
ContentTreeWidget::ContentTreeWidget(QWidget *parent) : QTreeWidget(parent) {
  setAcceptDrops(true);
}
...
ContentLabel::ContentLabel(QWidget *parent) : QLabel(parent) {
  setAcceptDrops(true);
}
