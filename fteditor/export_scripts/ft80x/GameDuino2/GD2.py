...
def run(name, document, ram):
    ...
    if not device_type == 2048:
        ...
        exportScriptName = exportScriptPath[0] + "export_GameDuino2.py"
        exportScript = imp.load_source("export_GameDuino2", exportScriptName)
        return exportScript.run(name, document, ram)