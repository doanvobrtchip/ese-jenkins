...
void DlEditor::reloadDisplayList(bool fromEmulator) {
  ...
  for (int i = 0; i < (m_ModeMacro ? FT800EMU_MACRO_SIZE : FTEDITOR_DL_SIZE);
    ...
    if (!m_ModeMacro && m_DisplayListShared[i] == DISPLAY() && dcount > 0) {
      ...
      QString line =
          DlParser::toString(FTEDITOR_CURRENT_DEVICE, m_DisplayListShared[i]);
      //      m_DisplayListParsed[i] = DlParsed();
      // verify parsing ->
      DlParser::parse(FTEDITOR_CURRENT_DEVICE, m_DisplayListParsed[i], line,
                      m_ModeCoprocessor);
      uint32_t compiled =
          DlParser::compile(FTEDITOR_CURRENT_DEVICE, m_DisplayListParsed[i]);
...
void DlEditor::editorCursorPositionChanged(bool isActive) {
  ...
  if (m_DisplayListParsed[block.blockNumber()].ValidId &&
      m_CompleterIdentifiersActive) {
    m_CompleterIdentifiersActive = false;
    m_CompleterModel->setStringList(m_CompleterParams);
    m_Completer->popup()->hide();
  } else if (!m_DisplayListParsed[block.blockNumber()].ValidId &&
             !m_CompleterIdentifiersActive) {
    m_CompleterIdentifiersActive = true;
    m_CompleterModel->setStringList(m_CompleterIdentifiers);
    m_Completer->popup()->hide();
  ...
  for (int i = 0; i < m_CodeEditor->document()->blockCount(); i++) {
    ...
    if (dl.IdLeft != FTEDITOR_CO_COMMAND) {
      ...
      QStringList argNames =
          m_CoproCmdArgName.value(QString(dl.IdText.c_str()));
      ...
      for (int i = 0; i < comp.size(); i++) {
        if (i == comp.size() - 1 &&
            argNames.indexOf(QRegularExpression("^string.*")) != -1) {
        ...
        if (argItem.startsWith("string")) {
          ...
          if (argItem.startsWith("<u>")) {
            argItem.remove(0, 3);
            QString h =
                QString("0x%1").arg(dl.Parameter[j++].U, 8, 16, QChar('0'));
...
void DlEditor::documentBlockCountChanged(int newBlockCount) {
  ...
  lockDisplayList();
  for (int i = 0; i < newBlockCount &&
                  i < (m_ModeMacro ? FT800EMU_MACRO_SIZE : FTEDITOR_DL_SIZE);
       ++i) {
    parseLine(m_CodeEditor->document()->findBlockByNumber(i));
  }
...
void DlEditor::replaceLine(int line, const DlParsed &parsed, int combineId,
  ...
  if (linestr.contains("CMD_SKETCH", Qt::CaseInsensitive) &&
      !m_adjustingCmdSketch) {
...
void DlEditor::adjustCmdSketch(DlParsed pre, DlParsed cur) {
  ...
  // adjust x, y
  if ((pre.Parameter[0].I != cur.Parameter[0].I) ||
      (pre.Parameter[1].I != cur.Parameter[1].I)) {
    adjustXY(pre, cur);
  }

  // adjust w, h
  if ((pre.Parameter[2].I != cur.Parameter[2].I) ||
      (pre.Parameter[3].I != cur.Parameter[3].I)) {
    // adjust cmd_memzero
...
void DlEditor::adjustCmdMemzero(DlParsed pre, DlParsed cur) {
  ...
  int prevNum =
      memsize(pre.Parameter[2].I, pre.Parameter[3].I, pre.Parameter[5].I);
  int curNum =
      memsize(cur.Parameter[2].I, cur.Parameter[3].I, cur.Parameter[5].I);
  ...
  for (int i = 0; i < m_CodeEditor->blockCount(); i++) {
    ...
    if (dl.ValidId && (dl.IdRight | 0xFFFFFF00) == CMD_MEMZERO) {
      ...
      if (dl.Parameter[0].I == pre.Parameter[4].I ||
          dl.Parameter[1].I == prevNum) {
...
void DlEditor::editingLine(QTextBlock block, bool force, bool isActive) {
  ...
  if (m_PropertiesEditor->getEditWidgetSetter() != this ||
      ...
      !m_DisplayListParsed[block.blockNumber()].ValidId ||
      force)  // Necessary for the "Unknown command" info message
  {
...
void DlEditor::setup(QObject *obj) {
  if (m_MainWindow->viewport() &&
      (obj == m_MainWindow->viewport() || obj == nullptr)) {
