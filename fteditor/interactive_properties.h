...
class InteractiveProperties : public QGroupBox {
 ...
 private:
  ...
  void addLabeledWidget(const QString &label, QWidget *widget0,
                        QWidget *widget1);

  void addXY(int x, int y, int minim, int maxim,
             QString label = QString("XY: "));
  ...
  void addSpinBox(int index, int minim, int maxim, const QString &label,
                  const QString &undoMessage, bool readOnly = false);
  void addSpinBoxUInt32(int index, double minim, double maxim,
                        const QString &label, const QString &undoMessage);
  void addSpinBox16(int index, int minim, int maxim, const QString &label,
                    const QString &undoMessage);
  void addSpinBox256(int index, int minim, int maxim, const QString &label,
                     const QString &undoMessage);
  void addSpinBoxForBitmapTransform(int index, int minim, int maxim,
                                    const QString &label,
                                    const QString &undoMessage);
  void addSpinBox65536(int index, int minim, int maxim, const QString &label,
                       const QString &undoMessage);
  void addSpinBoxAngle65536(int index, int minim, int maxim,
                            const QString &label, const QString &undoMessage);
  void addSpinBoxVertexFormat(int index, int minim, int maxim,
                              const QString &label, const QString &undoMessage);
  ...
  QComboBox *addComboBox(int index, const char **items, int nb,
                         const QString &label, const QString &undoMessage);
  QComboBox *addComboBox(int index, const char **items, int begin, int end,
                         const QString &label, const QString &undoMessage);
  QComboBox *addComboBox(int index, const int *toEnum, int toEnumSz,
                         const int *toIntf, const char **toString,
                         int toIntfStringSz, const QString &label,
                         const QString &undoMessage);
  void addBlendFunction(int blend, const QString &label,
                        const QString &undoMessage);
  void addCompareFunction(int compare);
  void addStencilOperation(int operation, const QString &label,
                           const QString &undoMessage);
  ...
  void addSwizzle(int swizzle, const QString &label,
                  const QString &undoMessage);
  void addBitmapWrap(int wrap, const QString &label,
                     const QString &undoMessage);
