...
struct ContentInfo {
  ...
  enum StorageType { File, Embedded, Flash };

  // ContentInfo::FlashMap functionality:
  // Similar to Raw, but takes a .map file as input, and renames the .map
  // extension to .bin to get the source data Links to specific file by means of
  // name. Error in case name no longer exists in the map
  ...
  StorageType DataStorage;  // Whether to use embedded header or file - only
                            // relevant for code generator scripts, or whether
                            // this is loaded into flash
  bool DataCompressed;      // Use compressed data source when embedding - only
                            // relevant for code generator scripts
...
class ContentManager : public QWidget {
 ...
 public:
  ...
  // Editor utilities
  // Find handle related to content address, return -1 on failure // TODO:
  // depend on current editor line
  ...
  // Find a free handle, return -1 when no free handle available // TODO: depend
  // on current editor line as fallback
  ...
  void editorUpdateHandle(ContentInfo *contentInfo, DlEditor *dlEditor,
                          bool updateSize);
  ...
  void changeConverter(ContentInfo *contentInfo,
  ...
  void changeMemoryAddress(ContentInfo *contentInfo, int value,
                           bool internal = false);
  void changeDataStorage(ContentInfo *contentInfo,
                         ContentInfo::StorageType value);
  void changeDataCompressed(ContentInfo *contentInfo, bool value);
  void changeFlashAddress(ContentInfo *contentInfo, int value,
                          bool internal = false);

  // Lock to call when editing/moving content files from qt thread, when reading
  // content from non-qt threads
  void lockContent() { s_Mutex.lock(); }
  void unlockContent() { s_Mutex.unlock(); }

  // Get
  inline static const std::vector<QString> &getFileExtensions() {
    return s_FileExtensions;
  }
 ...
 private:
  ...
  ContentInfo *m_CurrentPropertiesContent;  // This pointer may be invalid. Only
                                            // use to compare with current.
