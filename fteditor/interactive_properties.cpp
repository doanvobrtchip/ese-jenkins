...
InteractiveProperties::InteractiveProperties(MainWindow *parent)
    : QGroupBox(parent),
      m_MainWindow(parent),
      m_LineEditor(NULL),
      m_LineNumber(0) {
...
void InteractiveProperties::clear() {
  ...
  for (std::vector<QWidget *>::iterator it(m_CurrentWidgets.begin()),
       end(m_CurrentWidgets.end());
       it != end; ++it) {
    delete (*it);
  }
  m_CurrentWidgets.clear();
  m_CurrentProperties.clear();
  for (std::vector<QLayout *>::iterator it(m_CurrentLayouts.begin()),
       end(m_CurrentLayouts.end());
       it != end; ++it) {
    delete (*it);
  }
...
void InteractiveProperties::addLabeledWidget(const QString &label,
                                             QWidget *widget) {
...
void InteractiveProperties::addXY16(int x, int y, int minim, int maxim) {
  PropertiesSpinBox16 *propX =
      new PropertiesSpinBox16(this, "Set x position", x);
  propX->setMinimum(minim);
  propX->setMaximum(maxim);
  PropertiesSpinBox16 *propY =
      new PropertiesSpinBox16(this, "Set y position", y);
  propY->setMinimum(minim);
  propY->setMaximum(maxim);
...
void InteractiveProperties::addXYVertexFormat(int x, int y, int minim,
                                              int maxim) {
  PropertiesSpinBoxVertexFormat *propX =
      new PropertiesSpinBoxVertexFormat(this, "Set x position", x);
  propX->setMinimum(minim);
  propX->setMaximum(maxim);
  PropertiesSpinBoxVertexFormat *propY =
      new PropertiesSpinBoxVertexFormat(this, "Set y position", y);
  propY->setMinimum(minim);
  propY->setMaximum(maxim);
  addLabeledWidget("XY: ", propX, propY);
  m_CurrentProperties.push_back(propX);
...
void InteractiveProperties::addHandle(int handle, bool font) {
  PropertiesSpinBox *propHandle = new PropertiesSpinBox(
      this, font ? "Set font" : "Set handle", handle);  // TODO: Handle combobox
...
void InteractiveProperties::addOptions(int options, uint32_t flags,
                                       bool flatOnly, bool noClock) {
#define ADD_OPTIONS_CHECKBOX(opt)                                \
  {                                                              \
    PropertiesCheckBox *chb =                                    \
        new PropertiesCheckBox(this, "Set " #opt, options, opt); \
  ...
  #define OPT_SIGNED           256UL <- special case (when CMD_NUMBER, probably
  whenever no NOBACK) #define OPT_DITHER           256UL <- special case, bt817
  (when CMD_LOADIMAGE, probably whenever MONO) #define OPT_FLAT 256UL #define
  OPT_CENTERX          512UL #define OPT_CENTERY          1024UL #define
  OPT_CENTER           1536UL ---- #define OPT_RIGHTX           2048UL #define
  OPT_NOBACK           4096UL #define OPT_FILL             8192UL -- BT815
  #define OPT_NOTICKS          8192UL
  #define OPT_NOHM             16384UL
  #define OPT_NOPOINTER        16384UL <- special case (when NOHM this but not
  NOSECS) #define OPT_NOSECS           32768UL #define OPT_NOHANDS 49152UL ----
*/
  if (flags & OPT_MONO) {
...
void InteractiveProperties::addCharacter(int character) {
  PropertiesLineEditChar *propCharacter =
      new PropertiesLineEditChar(this, "Set character", character);
...
void InteractiveProperties::addStream(int stream) {
  ...
  PropertiesLineEdit *propText =
      new PropertiesLineEdit(this, "Set stream", stream);
  addLabeledWidget("Stream: ", propText);
  m_CurrentProperties.push_back(propText);

  // Help message
  addHelp(
      tr("<p><i>Direct filepath</i>:<br>Enter a relative or absolute filepath. "
         "String formatted in backslash escape format, use \\\\ for regular "
         "backslash.</p>"
         "<p><i>Content Manager</i>:<br>Enter the full content name suffixed "
         "with .raw or .bin (eg. content/catvideo.raw) for raw or compressed "
...
void InteractiveProperties::addValueSliderDyn(int val, int maxim) {
  PropertiesSliderDyn *propVal =
      new PropertiesSliderDyn(this, "Set value", val, maxim);
  addLabeledWidget("Value: ", propVal);
  m_CurrentProperties.push_back(propVal);
  propVal->ready();
}
...
void InteractiveProperties::addValueSliderDynSub(int val, int sub, int maxim) {
  PropertiesSliderDynSub *propVal =
      new PropertiesSliderDynSub(this, "Set value", val, sub, maxim);
  addLabeledWidget("Value: ", propVal);
  m_CurrentProperties.push_back(propVal);
  propVal->ready();
}
...
void InteractiveProperties::addSizeSubDynSlider(int size, int clip, int maxim) {
  PropertiesSliderDynSubClip *propSize =
      new PropertiesSliderDynSubClip(this, "Set size", size, clip, maxim);
...
void InteractiveProperties::addRangeMaximum(int range, int maxim) {
  PropertiesSpinBox *propRange =
      new PropertiesSpinBox(this, "Set range", range);
...
void InteractiveProperties::addWidth(int width, int minim, int maxim) {
  PropertiesSpinBox *propWidth =
      new PropertiesSpinBox(this, "Set width", width);
...
void InteractiveProperties::addRadius(int radius, int minim, int maxim) {
  PropertiesSpinBox *propRadius =
      new PropertiesSpinBox(this, "Set radius", radius);
...
void InteractiveProperties::addSpinBox(int index, int minim, int maxim,
                                       const QString &label,
...
void InteractiveProperties::addSpinBoxUInt32(int index, double minim,
                                             double maxim, const QString &label,
                                             const QString &undoMessage) {
  PropertiesSpinBoxUInt32 *prop =
      new PropertiesSpinBoxUInt32(this, undoMessage, index);
  prop->setMinimum(minim);
  prop->setMaximum(maxim);
...
void InteractiveProperties::addSpinBox16(int index, int minim, int maxim,
                                         const QString &label,
                                         const QString &undoMessage) {
...
void InteractiveProperties::addSpinBox256(int index, int minim, int maxim,
                                          const QString &label,
                                          const QString &undoMessage) {
  PropertiesSpinBox256 *prop =
      new PropertiesSpinBox256(this, undoMessage, index);
  prop->setMinimum(minim);
  prop->setMaximum(maxim);
...
void InteractiveProperties::addSpinBoxForBitmapTransform(
    int index, int minim, int maxim, const QString &label,
    const QString &undoMessage) {
  PropertiesSpinBoxForBitmapTransform *prop =
      new PropertiesSpinBoxForBitmapTransform(this, undoMessage, index);
  prop->setMinimum(minim);
  prop->setMaximum(maxim);
  prop->setSingleStep(256);
  addLabeledWidget(label, prop);
...
void InteractiveProperties::addSpinBox65536(int index, int minim, int maxim,
                                            const QString &label,
                                            const QString &undoMessage) {
  PropertiesSpinBox65536 *prop =
      new PropertiesSpinBox65536(this, undoMessage, index);
  prop->setMinimum(minim);
  prop->setMaximum(maxim);
...
void InteractiveProperties::addSpinBoxAngle65536(int index, int minim,
                                                 int maxim,
                                                 const QString &label,
                                                 const QString &undoMessage) {
  PropertiesSpinBoxAngle65536 *prop =
      new PropertiesSpinBoxAngle65536(this, undoMessage, index);
  prop->setMinimum(minim);
  prop->setMaximum(maxim);
...
void InteractiveProperties::addSpinBoxVertexFormat(int index, int minim,
                                                   int maxim,
                                                   const QString &label,
                                                   const QString &undoMessage) {
  PropertiesSpinBoxVertexFormat *prop =
      new PropertiesSpinBoxVertexFormat(this, undoMessage, index);
  prop->setMinimum(minim);
  prop->setMaximum(maxim);
  addLabeledWidget(label, prop);
  m_CurrentProperties.push_back(prop);
...
void InteractiveProperties::addCheckBox(int index, const QString &label,
                                        const QString &undoMessage) {
  PropertiesCheckBox *prop =
      new PropertiesCheckBox(this, undoMessage, index, 0x01);
  addLabeledWidget(label, prop);
  m_CurrentProperties.push_back(prop);
}

void InteractiveProperties::addWidth16(int width, int minim, int maxim,
                                       bool size) {
  PropertiesSpinBox16 *propWidth =
      new PropertiesSpinBox16(this, size ? "Set size" : "Set width", width);
  propWidth->setMinimum(minim);
  propWidth->setMaximum(maxim);
...
void InteractiveProperties::addAlphaHex(int alpha) {
  PropertiesSpinBoxAlphaHex *prop =
      new PropertiesSpinBoxAlphaHex(this, tr("Set alpha"), alpha);
...
void InteractiveProperties::addByteFlag(int flag, const QString &undoMessage) {
  ...
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x80);
    hbox->addWidget(chb);
  ...
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x40);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x20);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x10);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x08);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x04);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x02);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
  {
    PropertiesCheckBox *chb =
        new PropertiesCheckBox(this, undoMessage, flag, 0x01);
    hbox->addWidget(chb);
    m_CurrentWidgets.push_back(chb);
    m_CurrentProperties.push_back(chb);
  }
...
QComboBox *InteractiveProperties::addComboBox(int index, const char **items,
                                              int nb, const QString &label,
                                              const QString &undoMessage) {
...
QComboBox *InteractiveProperties::addComboBox(int index, const char **items,
                                              ...
                                              const QString &label,
                                              const QString &undoMessage) {
  PropertiesComboBox *prop =
      new PropertiesComboBox(this, undoMessage, index, begin);
  for (int i = begin; i < end; ++i) prop->addItem(items[i]);
  addLabeledWidget(label, prop);
  m_CurrentProperties.push_back(prop);
  prop->ready();
  return prop;
...
QComboBox *InteractiveProperties::addComboBox(int index, const int *toEnum,
                                              int toEnumSz, const int *toIntf,
                                              const char **toString,
                                              int toIntfStringSz,
                                              const QString &label,
                                              const QString &undoMessage) {
  PropertiesRemapComboBox *prop = new PropertiesRemapComboBox(
      this, undoMessage, index, toIntf, toIntfStringSz, toEnum, toEnumSz);
  for (int i = 0; i < toEnumSz; ++i) prop->addItem(toString[toEnum[i]]);
  addLabeledWidget(label, prop);
  m_CurrentProperties.push_back(prop);
  prop->ready();
  return prop;
...
void InteractiveProperties::addBlendFunction(int blend, const QString &label,
                                             const QString &undoMessage) {
...
void InteractiveProperties::addCompareFunction(int compare) {
  addComboBox(compare, g_DlEnumCompare, DL_ENUM_COMPARE_NB,
              "Func: ", "Set func");
}

void InteractiveProperties::addStencilOperation(int operation,
                                                const QString &label,
                                                const QString &undoMessage) {
  addComboBox(operation, g_DlEnumStencil, DL_ENUM_STENCIL_NB, label,
              undoMessage);
}

void InteractiveProperties::addPrimitive(int primitive) {
  addComboBox(primitive, g_DlEnumPrimitive, 1, DL_ENUM_PRIMITIVE_NB,
              "Primitive: ", "Set primitive");
}

void InteractiveProperties::addBitmapFormat(int format) {
  addComboBox(format, g_BitmapFormatFromIntf[FTEDITOR_CURRENT_DEVICE],
...
void InteractiveProperties::addBitmapFormatCmdSketch(int format) {
  ...
  addComboBox(format, toEnum, 2, toIntf, bfString, 4, tr("Format") + ": ",
              tr("Set bitmap format"));
}
...
void InteractiveProperties::addExtFormat(int format) {
  ...
  QComboBox *comboBox =
      addComboBox(format, g_ExtFormatFromIntf[FTEDITOR_CURRENT_DEVICE],
  ...
  for (int i = 0; i < comboBox->count(); ++i)
    comboBox->setItemText(
        i, comboBox->itemText(i).replace("COMPRESSED_RGBA_", ""));
}

void InteractiveProperties::addSwizzle(int swizzle, const QString &label,
                                       const QString &undoMessage) {
  addComboBox(swizzle, g_DlEnumSwizzle, DL_ENUM_SWIZZLE_NB, label, undoMessage);
}

void InteractiveProperties::addBitmapWrap(int wrap, const QString &label,
                                          const QString &undoMessage) {
  addComboBox(wrap, g_DlEnumBitmapWrap, DL_ENUM_BITMAP_WRAP_NB, label,
              undoMessage);
}

void InteractiveProperties::addBitmapFilter(int filter) {
  addComboBox(filter, g_DlEnumBitmapFilter, DL_ENUM_BITMAP_FILTER_NB,
              tr("Filter") + ": ", tr("Set bitmap filter"));
}

void InteractiveProperties::addAnimLoop(int loop) {
  addComboBox(loop, g_DlEnumAnimLoop, DL_ENUM_ANIM_LOOP_NB, tr("Loop") + ": ",
              tr("Set animation loop"));
}

void InteractiveProperties::addAddressFlashOpt(int index, bool negative) {
  ;
  {
    PropertiesSpinBoxAddressFlashOpt *prop =
        new PropertiesSpinBoxAddressFlashOpt(this, tr("Set address"), index,
                                             negative);
  ...
  {
    PropertiesCheckBox *prop =
        new PropertiesCheckBox(this, tr("Set flash"), index, 0x800000);
    addLabeledWidget("Flash: ", prop);
    m_CurrentProperties.push_back(prop);
  }
}

void InteractiveProperties::addAddressFlash(int index) {
  PropertiesSpinBoxAddressFlash *prop =
      new PropertiesSpinBoxAddressFlash(this, tr("Set flash address"), index);
...
void InteractiveProperties::addAddress(int index, bool negative, bool readOut) {
  PropertiesSpinBoxAddress *prop =
      new PropertiesSpinBoxAddress(this, tr("Set address"), index, negative);
  prop->setReadOut(readOut);
...
void InteractiveProperties::addAddress64ByteAligned(int index) {
  PropertiesSpinBoxAddress *prop =
      new PropertiesSpinBoxAddress(this, tr("Set address"), index, false);
...
void InteractiveProperties::addMemorySize(int size) {
  PropertiesSpinBoxAddress *prop =
      new PropertiesSpinBoxAddress(this, tr("Set size"), size, false);
  addLabeledWidget("Size: ", prop);
  m_CurrentProperties.push_back(prop);
  prop->done();
}

void InteractiveProperties::addCaptureButton(const QString &text,
                                             const QString &undoMessage) {
  PropertiesCaptureButton *prop =
      new PropertiesCaptureButton(this, text, undoMessage);
  ...
  if (editor) {
    ...
    if (parsed.ValidId) {
      ...
      if (parsed.IdText.size() > 0) {
        QString message =
            QString("Unknown command '<i>%1</i>'").arg(parsed.IdText.c_str());
...
void InteractiveProperties::setProperties(int idLeft, int idRight,
                                          DlEditor *editor) {
  bool ok = false;
  ...
  if (idLeft == FTEDITOR_DL_VERTEX2F) {
    ...
    ok = true;
  } else if (idLeft == 0xFFFFFF00)
    switch (idRight | 0xFFFFFF00) {
      case CMD_DLSTART: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_DLSTART."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
        }
        ok = true;
      ...
      case CMD_TEXT: {
        ...
        if (editor) {
          ...
          addXY(0, 1, FTEDITOR_COORD_MIN, FTEDITOR_COORD_MAX);
          addHandle(2, true);
          addOptions(3, OPT_CENTER | OPT_RIGHTX | OPT_FILL | OPT_FORMAT, false,
                     true);
          addText(4);
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
      ...
      case CMD_MEMWRITE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_MEMWRITE."));
        if (editor) {
          setTitle("CMD_MEMWRITE");
      ...
      case CMD_LOADIMAGE: {
        if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810)
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_CMD_LOADIMAGE_FT810."));
        else
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_CMD_LOADIMAGE."));
        if (editor) {
      ...
      case CMD_LOADIDENTITY: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_LOADIDENTITY."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
        }
        ok = true;
      ...
      case CMD_TRANSLATE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_TRANSLATE."));
        if (editor) {
          setTitle("CMD_TRANSLATE");
          addSpinBox65536(0, 0x80000000, 0x7FFFFFFF,
                          "X: ", tr("Set x translation"));
          addSpinBox65536(1, 0x80000000, 0x7FFFFFFF,
                          "Y: ", tr("Set y translation"));
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
        break;
      ...
      case CMD_ROTATE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_ROTATE."));
        if (editor) {
          setTitle("CMD_ROTATE");
          addSpinBoxAngle65536(0, 0x80000000, 0x7FFFFFFF, tr("Angle") + ": ",
                               tr("Set angle"));
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_SCREENSAVER: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_SCREENSAVER."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
        }
        ok = true;
      ...
      case CMD_SNAPSHOT2: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_SNAPSHOT2."));
        if (editor) {
          setTitle("CMD_SNAPSHOT2");
          addComboBox(0, g_SnapshotFormatFromIntf[FTEDITOR_CURRENT_DEVICE],
      ...
      case CMD_PLAYVIDEO: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_PLAYVIDEO."));
        if (editor) {
          setTitle("CMD_PLAYVIDEO");
          if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
            addOptions(0, OPT_NOTEAR | OPT_FULLSCREEN | OPT_MEDIAFIFO |
                              OPT_SOUND | OPT_FLASH | OPT_OVERLAY | OPT_NODL);
          } else {
            addOptions(0,
                       OPT_NOTEAR | OPT_FULLSCREEN | OPT_MEDIAFIFO | OPT_SOUND);
          }
          addStream(1);
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
      ...
      case CMD_SETFONT2: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_SETFONT2."));
        if (editor) {
          setTitle("CMD_SETFONT2");
          addSpinBox(0, 0, FTEDITOR_FONTHANDLE_MAX, tr("Font") + ": ",
                     tr("Set font"));
          addAddress(1, true);
          addSpinBox(2, 0, 255, tr("First character") + ": ",
                     tr("Set first character"));
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
        break;
      ...
      case CMD_FLASHUPDATE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FLASHUPDATE."));
        if (editor) {
      ...
      case CMD_FLASHDETACH: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FLASHDETACH."));
        if (editor) {
      ...
      case CMD_FLASHATTACH: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FLASHATTACH."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_FLASHSPIDESEL: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FLASHSPIDESEL."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_FLASHSOURCE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FLASHSOURCE."));
        if (editor) {
      ...
      case CMD_ROTATEAROUND: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_ROTATEAROUND."));
        if (editor) {
          setTitle("CMD_ROTATEAROUND");
          addXY(0, 1, FTEDITOR_COORD_MIN, FTEDITOR_COORD_MAX);
          addSpinBoxAngle65536(2, 0x80000000UL, 0x7FFFFFFFUL,
                               tr("Angle") + ": ", tr("Set angle"));
          addSpinBox65536(3, 0x80000000UL, 0x7FFFFFFFUL, tr("Scale") + ": ",
                          tr("Set scale"));
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_ANIMSTARTRAM: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_ANIMSTARTRAM."));
        if (editor) {
      ...
      case CMD_RUNANIM: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_RUNANIM."));
        if (editor) {
          setTitle("CMD_RUNANIM");
          addSpinBoxUInt32(0, 0, 0xFFFFFFFF, tr("Wait Mask:"),
                           tr("Set Wait Mask"));
          addAddress(1, true);
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
      ...
      case CMD_ANIMFRAMERAM: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_ANIMFRAMERAM."));
        if (editor) {
      ...
      case CMD_VIDEOSTARTF: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_VIDEOSTARTF."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
        }
        ok = true;
      ...
      case CMD_BITMAP_TRANSFORM: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_BITMAP_TRANSFORM."));
        if (editor) {
      ...
      case CMD_WAIT: {
        m_MainWindow->propertiesEditor()->setInfo(tr("DESCRIPTION_CMD_WAIT."));
        if (editor) {
          setTitle("CMD_WAIT");
          addSpinBoxUInt32(0, 0, 1000000, tr("Delay Time:"),
                           tr("Set Delay Time"));
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_NEWLIST: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_NEWLIST."));
        if (editor) {
          setTitle("CMD_NEWLIST");
          addAddress(0, false);
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_CALLLIST: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_CALLLIST."));
        if (editor) {
          setTitle("CMD_CALLLIST");
          addAddress(0, false);
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
      ...
      case CMD_APILEVEL: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_APILEVEL."));
        if (editor) {
          setTitle("CMD_APILEVEL");
          addSpinBox(0, 1, 2, tr("API Level:"), "Set API Level");
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
        break;
      ...
      case CMD_CALIBRATESUB: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_CALIBRATESUB."));
        if (editor) {
      ...
      case CMD_FONTCACHE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FONTCACHE."));
        if (editor) {
          setTitle("CMD_FONTCACHE");
          addHandle(0, true);
          addAddress(1, false);
          addMemorySize(2);
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
      ...
      case CMD_FONTCACHEQUERY: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_FONTCACHEQUERY."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
        }
        ok = true;
      ...
      case CMD_PCLKFREQ: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_PCLKFREQ.", "info"));
        if (editor) {
          setTitle("CMD_PCLKFREQ");
          addSpinBoxUInt32(0, 0, 0xFFFFFFFF, tr("Target Frequency"),
                           tr("Set Target frequency"));
      ...
      case CMD_HSF: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CMD_HSF.", "info"));
        if (editor) {
          setTitle("CMD_HSF");
          int *ram32 = reinterpret_cast<int *>(BT8XXEMU_getRam(g_Emulator));
          int currentWidth =
              ram32[reg(FTEDITOR_CURRENT_DEVICE, FTEDITOR_REG_HSIZE) >> 2];
          addSpinBox(0, 0, currentWidth, tr("Pixel Width"),
                     tr("Set Pixel Width"));
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
    ...
    switch (idRight) {
      ...
      case FTEDITOR_DL_CLEAR_COLOR_RGB: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_CLEAR_COLOR_RGB."));
        if (editor) {
      ...
      case FTEDITOR_DL_BITMAP_LAYOUT: {
        if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_FT810)
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_LAYOUT_FT810."));
        else
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_LAYOUT."));
        if (editor) {
      ...
      case FTEDITOR_DL_BITMAP_TRANSFORM_A: {
        if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815)
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_A_BT815."));
        else
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_A."));
        if (editor) {
          setTitle("BITMAP_TRANSFORM_A");
          if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
            addCheckBox(0, "Precision: ", "Set transform precision A");
            addSpinBoxForBitmapTransform(
                1, 0xFFFF0000, 0xFFFF,
                "A: ", "Set transform A");  // TODO: Precision-aware SpinBox
          } else {
      ...
      case FTEDITOR_DL_BITMAP_TRANSFORM_B: {
        if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815)
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_B_BT815."));
        else
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_B."));
        if (editor) {
          ...
          if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
            ...
            addSpinBoxForBitmapTransform(
                1, 0xFFFF0000, 0xFFFF,
                "B: ", "Set transform B");  // TODO: Precision-aware SpinBox
          } else {
      ...
      case FTEDITOR_DL_BITMAP_TRANSFORM_C: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_BITMAP_TRANSFORM_C."));
        if (editor) {
      ...
      case FTEDITOR_DL_BITMAP_TRANSFORM_D: {
        if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815)
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_D_BT815."));
        else
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_D."));
        if (editor) {
          ...
          if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
            ...
            addSpinBoxForBitmapTransform(
                1, 0xFFFF0000, 0xFFFF,
                "D: ", "Set transform D");  // TODO: Precision-aware SpinBox
          } else {
      ...
      case FTEDITOR_DL_BITMAP_TRANSFORM_E: {
        if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815)
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_E_BT815."));
        else
          m_MainWindow->propertiesEditor()->setInfo(
              tr("DESCRIPTION_BITMAP_TRANSFORM_E."));
        if (editor) {
          ...
          if (FTEDITOR_CURRENT_DEVICE >= FTEDITOR_BT815) {
            ...
            addSpinBoxForBitmapTransform(
                1, 0xFFFF0000, 0xFFFF,
                "E: ", "Set transform E");  // TODO: Precision-aware SpinBox
          } else {
      ...
      case FTEDITOR_DL_BITMAP_TRANSFORM_F: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_BITMAP_TRANSFORM_F."));
        if (editor) {
      ...
      case FTEDITOR_DL_RESTORE_CONTEXT: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_RESTORE_CONTEXT."));
        if (editor) {
          m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
        }
        ok = true;
      ...
      case FTEDITOR_DL_VERTEX_FORMAT: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_VERTEX_FORMAT."));
        if (editor) {
          setTitle("VERTEX_FORMAT");
          addSpinBox(0, 0, 4,
                     "Fractional: ", "Set the number of fractional bits");
          m_MainWindow->propertiesEditor()->setEditWidget(this, false, editor);
        }
        ok = true;
        break;
      ...
      case FTEDITOR_DL_VERTEX_TRANSLATE_X: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_VERTEX_TRANSLATE_X."));
        if (editor) {
      ...
      case FTEDITOR_DL_VERTEX_TRANSLATE_Y: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_VERTEX_TRANSLATE_Y."));
        if (editor) {
      ...
      case FTEDITOR_DL_BITMAP_EXT_FORMAT: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_BITMAP_EXT_FORMAT."));
        if (editor) {
      ...
      case FTEDITOR_DL_BITMAP_SWIZZLE: {
        m_MainWindow->propertiesEditor()->setInfo(
            tr("DESCRIPTION_BITMAP_SWIZZLE_FORMAT."));
        if (editor) {
  ...
  if (!ok) {
    m_MainWindow->propertiesEditor()->setError(
        tr("</i>Not yet implemented.</i>"));
    m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
  }
  if (!editor) {
    m_MainWindow->propertiesEditor()->setEditWidget(NULL, false, editor);
...
void InteractiveProperties::modifiedEditorLine() {
  ...
  for (std::vector<PropertiesWidget *>::iterator
           it(m_CurrentProperties.begin()),
       end(m_CurrentProperties.end());
       it != end; ++it) {
